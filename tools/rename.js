...
function wende(datei, kind, map, opt = {}) {
  const src = fs.readFileSync(datei, 'utf8');
  const f = kind === 'ident' ? ersetzeIdent : kind === 'string' ? ersetzeString
    : kind === 'kommentar' ? nurKommentare
    : kind === 'qualifiziert' ? ersetzeQualifiziert
    : kind === 'regex' ? nurRegex : ersetzeInString;
...
if (require.main === module) {
  ...
  if (kind === 'batch') {
    ...
    for (const schritt of plan) {
      const t = wende(schritt.datei, schritt.kind, schritt.map, { ...opt, ...schritt.opt });
      gesamt += t;
      console.log(`${schritt.datei}: ${schritt.kind} — ${t} Treffer (${Object.keys(schritt.map).length} Namen)`);
    }
    ...
    const t = wende(datei, kind, { [von]: nach }, opt);
