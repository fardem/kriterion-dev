...
| **0.41.0** *MINOR* — **GEBAUT am 23. September 2026** | **Backup, Videos in Kommentaren und der Prüfstand ohne feste Wartezeiten.** *Punkt 41 dieses Blatts, vollständig, dazu 48 bis 51. Bis zum Auftrag hieß die Runde „Der Prüfstand wartet auf eine Bedingung statt auf die Uhr".* |
| **0.42.0** und **0.43.0** *MINOR* — **GEPLANT am 21. September 2026, 0.42.0 GEBAUT am 25. September 2026** | **Dokumente über einen Document Server ansehen und bearbeiten.** *Aus keinem Punkt dieses Blatts — eine Frage des Betreibers vom selben Tag.* |
...
| ~~**Ein echter Teillauf**~~ | **GEBAUT am 15. September 2026 mit 0.34.0.** Der Treiber liest die Gruppennamen aus dem Quelltext jedes Moduls und startet nur die Module, auf die der Filter passt. Gemessen: `node testbench.js Schluesselwechsel` braucht **14 s** statt 350 s. Ein Filter ohne Treffer bleibt rot. **Offen bleibt der Rundlauf**: seine 176 Gruppen liegen in einem Modul, ein Teillauf über „Rechte" kostet deshalb weiter dessen 108 Sekunden |
...
| **Eine Messung im echten Browser** | **weiter zurückgestellt.** Entschieden am 15. September 2026 beim Bauen von 0.34.0: nein. Die Runde hat keine neue Abhängigkeit aufgenommen; der Grund für die Aufteilung war der Speicher, und der ist ohne Browser gelöst. Die Frage von 0.15.1 bleibt unbeantwortet: wie viele Zusagen der Oberfläche stehen nur in jsdom und nicht im Browser |
...
## 52. Das Backup der `.env` vor dem Schlüsselwechsel steht in keiner Ignorierliste

**Art: Fehler** *(Betrieb)* **· Herkunft: Bau von 0.41.0, 23. September 2026 ·
Einschätzung: klein · Fahrplan: offen**

`keytool.sh change` legt `.env.before-key-change-<Marke>` neben die `.env`.
Die Datei trägt den alten Schlüssel. Sie steht weder in `.gitignore` noch in
`.dockerignore`: `COPY . .` im `Dockerfile` nimmt sie beim nächsten Bau mit
ins Image. Unter dem alten Namen `.env.vor-schluesselwechsel-<Marke>` war es
genauso.
