...
**0.8.10 — Fingerprint `48fe44e7`**

Der Fingerprint ist **zuletzt** gebildet worden, nach der letzten Änderung an
...
fällt aus dieser Version **nichts** an: der Fingerprint ist eine Ableitung beim
...
- `COPY package.json package-lock.json ./` statt `COPY package.json ./` — die
  Bauphase bekommt das Lockfile überhaupt erst zu sehen.
...
- Die zweite Stufe holt sich weiterhin alles über `COPY --from=builder` und
  bringt das Lockfile damit ohnehin mit.
...
**Punkt 3 — der Versions-Fingerprint.** Der einzige Eingriff in den
Anwendungscode dieser Runde.
- `crypto` am Dateianfang dazu.
- `dateienUnter(verzeichnis)` — läuft ein Verzeichnis rekursiv ab.
- `bildeFingerprint()` — SHA-256 über die Dateien, acht Zeichen. **Die Liste wird
  ...
  unsichtbar.
- `const FINGERPRINT = bildeFingerprint();` unmittelbar vor `app.listen`. Dort, weil
  `require.cache` erst nach allen `require`-Aufrufen vollständig ist.
- `GET /api/stats` nennt `fingerprint` als zweites Feld, direkt hinter `version`.
- Heute deckt der Fingerprint sieben Dateien ab: `server.js`, `db.js`, `auth.js`,
  `keys.js`, `anhaenge.js`, `package.json` und alles unter `public/`.

### `public/app.js` (+7/−1 Zeilen)

**Punkt 3.** Die Karte „Kennzahlen" bekommt eine Zeile `Fingerprint` hinter
...
**Der Mock in `baueDom()`** liefert `fingerprint: 'a1b2c3d4'` in der
...
`concurrency`-Block, der einen überholten Lauf auf demselben Branch ablöst.
...
**Der Fingerprint geht nach `/api/stats`, nicht nach `/api/config`.** Der Auftrag
...
3. Der Fingerprint nagelt den laufenden Dateisatz fest. Vor der Anmeldung nützt
...
`zugang.js` liegt im Image, läuft aber nie im Server und steht deshalb
**nicht** im Fingerprint. Ein veraltetes `zugang.js` fiele nicht auf. Der Fingerprint
...
| Image | `npm ci --omit=dev` ohne Übersetzer | `better-sqlite3-multiple-ciphers` |
|---|---|---|
...
nicht im Image, ein Fund dort träfe die Anwendung nicht. Aber sich darauf
...
überholten Lauf auf demselben Branch ab, und `cache: npm` an `setup-node` hält
...
Funktion liefe später und stünde dann nicht darin — der Fingerprint würde still
...
| Rückbau | Ergebnis |
|---|---|
| `package-lock.json` wird in `.dockerignore` aufgenommen | **1 rot** — *.dockerignore hält das Lockfile nicht zurück* |
...
| `version` im Lockfile auf `0.0.1` | **1 rot** — *Es gehört zu dieser package.json* |
| `sharp` fällt aus `packages` des Lockfiles | **1 rot** — *Es nennt jede Abhängigkeit* |
...
Lockfile nicht zurück" **grün** — eine leere Trefferliste macht jede
...
### Punkt 3 — der Versions-Fingerprint
...
| der Mock kennt `fingerprint` nicht mehr | **1 rot** |
...
Mock belegt Stolperstein 90: die Zeile wird weiterhin **gezeichnet**
...
ist nicht abgesenkt" **grün** — ein leerer String macht jede Verneinung
...
denselben Fingerprint; `pruefung.js`, `Doku/` und `zugang.js` bewegen ihn nicht;
...
Fingerprint bleibt vor der Anmeldung draussen" und „Die Kennzahlen nennen ihn
...
**Nichts.** Es ist kein Migrationscode entstanden. Der Fingerprint ist eine
...
Der Stand von 0.8.3 bleibt unverändert: `db.js`, `migration083()`, 18 Zeilen,
7 Prüfungen.
...
durch, das Image startet, `/api/config` antwortet mit `0.8.10`, Node 22.23.2,
`sharp` 0.35.3 auf libvips 8.18.3, `better-sqlite3-multiple-ciphers` lädt
gegen sqlite 3.49.2, im Laufzeit-Image liegt kein Übersetzer, und der Fingerprint
...
Die Krücke war ein Grund-Image aus dem **vollen** `node:22-bookworm`, das die
...
gemessen gegen das echte schlanke Grund-Image: das Image ist **101 MB** groß
...
gleichzeitig ein Image gebaut wurde. Drei Läufe hintereinander ohne Nebenlast
...
etwa 2–3 Minuten, ein Push auf einen Branch mit offener Anfrage läuft zweimal,
also rund 5 Minuten. Selbst auf Free sind das ungefähr **400 Pushes im
Monat**.

**Der Fingerprint deckt `zugang.js` nicht ab.** Bewusst so entschieden und
...
- **Projektstand, Abschnitt 9 (Versionsgeschichte):** 0.8.10 „Werkzeug" —
  alle fünf Punkte, Fingerprint `48fe44e7`.
...
- **Projektstand, Abschnitt 11:** neu bindend — *die Node-Version steht an
  ...
  lädt noch ausliefert, steht nicht im Fingerprint*.
- **README:** der Abschnitt über die Textstelle zum Gegenprüfen ist durch
  Punkt 3 überholt und gehört durch drei Sätze über den Fingerprint ersetzt.
  Dazu, was Punkt 1 am Einspielweg ändert (`npm ci`, Lockfile) und dass
  das Image jetzt auf Node 22 steht.
