...
**Kompakte Übergabe · Revision 63 · Stand 5. September 2026 · gebaut: Version 0.23.0**
...
sofort auf die Übersicht ging, bekam einen roten Kasten mit dem Wortlaut des
Browsers — „can't access property innerHTML" — über einen Vorgang, der jedes Mal
geglückt war* (Stolperstein 298). **Dieselbe Runde nimmt das Wort „gebacken" aus
dem ganzen Projekt**; was der Lauf tut, heißt jetzt **erneuern**, was eine
Ableitung herstellt, **erzeugen**, und was im Bild steckt, ist
**eingerechnet**. *Das Änderungsprotokoll 0.19.6 trägt beides.*

**0.19.5 in einem Satz: der Ausschnitt wird eingerechnet, nicht gezogen.** *PATCH
...
nicht schneidet, zeigt den Mittenschnitt. Beides gehört in dieselbe Runde.*
...
1,37fach hochgezogen —, der eingerechnete gibt **5,88×**, und zwar unabhängig
...
erzeugt die Kachel neu und antwortet erst danach; und der Bestandslauf erneuert den
vorhandenen Bestand mit. **Gemessen:** die Kachel wird in Summe **34,2 %
kleiner** (36 Vorlagen in zwölf Seitenverhältnissen), das Erzeugen kostet 157 ms
...
selbst.* **Erstens: die Bestandskarte war weiter langsam.** 0.19.1 hat die eine
von **zwei** Ursachen behoben; die zweite ist die **Spaltenlage**: `art` steht
in `photos` hinter drei Blobs, und wer sie aus dem Satz liest, muss dessen
Overflow-Ketten lesen und entschlüsseln — *gemessen 1338,8 ms gegen 0,1 ms aus
einem Index, und `MATERIALIZED` hilft dagegen nichts.* **Die ganze Route
`GET /api/stats`: 4698 ms vorher, 28,6 ms kalt und 4,4 ms warm nachher**, an
derselben 312-MB-Datei. **Zweitens: der engere Ausschnitt erreichte die Ränder
immer noch nicht** — `transform-origin` an der Kachel war richtig und blieb
wirkungslos, weil der **Betrachter** gar keinen anderen Wert als 50 zuließ: sein
Spielraum rechnete den Zoom nicht ein. **Drittens: die beiden Dialogtexte sind
kurz geworden.** **Und zwei Zahlen aus 0.19.1 sind berichtigt** — sie waren aus
dem Auftrag übernommen und nicht an der echten Tabellenform nachgefahren.
**KEINE DATENBANKSTUFE:** acht Migrationsblöcke, Austauschformat **12**,
`F_ROUTEN` **70** — *ein Index ist keine Stufe.* *Alles Weitere in Abschnitt 2
und Abschnitt 9.*
...
konnte.* **Der Befund aus 0.17.2 ist gefunden, nach drei Runden**, und es war
die ganze Zeit eine einzige Zeile: **`max-height: max-content`**. In Chromium
klemmt sie die Forderung einer kurzen Liste — *deshalb war der Befund zwei
Runden lang nicht nachstellbar, und deshalb sind zwei Runden lang Ursachen
gebaut worden, die es nicht gab.* **In der laufenden Instanz klemmt sie nicht:**
dort forderte JEDE Liste ihre zehn Zeilen, auch die leere. Gemeldet mit Bild und
Fingerprint `d3113d62`: die Karte „Zugänge" mit fünf Zeilen stand **728** Pixel
hoch, wo dieselbe Karte in Chromium **498** misst; der Papierkorb mit **leerer**
Liste stand allein in seiner Reihe und war trotzdem rund **570** hoch.
**An ihre Stelle treten zwei Zeilen ohne Schlüsselwort:** `flex: 0 1 auto` —
*so hoch wie mein Inhalt, und ich nehme mir nichts von der Kachel* — und
`max-height: <Deckel>rem` — *hier ist Schluss*. **Der Preis steht dazu:** eine
Liste nutzt die Höhe einer höheren Nachbarin nicht mehr aus; die Zusage aus
0.17.4 ist zurückgenommen. **Dazu das Raster des Sicherheitsprotokolls:** es
gehört jetzt der **Liste** und nicht mehr der einzelnen Zeile, damit die Namen
untereinander stehen — vorher vier verschiedene linke Kanten, jetzt eine.
*Alles Weitere in Abschnitt 2 und Abschnitt 9.*
...
stand dafür in `flex-basis`; ***dieser Mechanismus ist mit 0.17.5
zurückgenommen***, weil er an `max-height: max-content` hing und dieses
Schlüsselwort nicht überall klemmt. **Drei Maße
...
Bilderstreifen fragen jetzt zuerst, ob ihre Ansicht noch steht; die Bestätigung
„Bildausschnitt gespeichert" kommt trotzdem.*

**Davor: 0.19.5** — Fingerprint **`f228a06d`**, **5237
Prüfungen**, 532 Rückbauten.
*0.19.5 kehrt eine Entscheidung aus 0.19.4 um: der Bildausschnitt wird ab jetzt
am Server in die Kachel **gerechnet** statt im Browser aus ihr gezogen. Die
...
> **DER AUSSCHNITT WIRD BEIM SPEICHERN NEU ERZEUGT, UND DIE ANTWORT WARTET
...
> (Stolperstein 296). **Schlägt das Erzeugen fehl, ist der Ausschnitt trotzdem
...
> **Eingerechnet ändert sich der INHALT unter derselben Adresse**, und der
...
> Erzeugen zeigte eine Videokachel mit `zoom > 100` nach dieser Runde den
...
> Mittenschnitt. **Mit einer zugeschnittenen Kachel zeigt er ihn mit.** *Eine
...
> gemessen **1,4 s bei 312 MB**, danach steht er. *Gesichert werden muss
> deshalb nichts.*

*Davor, am 2. September:* **0.19.1** — Fingerprint **`b0c4da5b`**, **5104
Prüfungen**, **472 Rückbauten in der Liste**.
...
>
> **UND SIE HAT EINEN SATZ WIDERLEGT.** Zu Rückbau 433 steht im Protokoll „der Fall kommt am echten Bestand vor" — er stammt aus dem Auftrag und nicht aus einer Messung. **Kein einziges PNG ist liegen geblieben**, und achtzehn Laborversuche fanden kein Gegenbeispiel. *Die Berichtigung ist Punkt 9 von 0.19.1.*
...
Kachel) und `max-height: <Deckel>rem` (hier ist Schluss). Dazu wandert das
Raster des Sicherheitsprotokolls von der Zeile auf die Liste, damit die Namen
untereinander stehen.* **PATCH — nur `public/style.css` ist angefasst; die
Instanz kann danach nichts, was sie vorher nicht konnte.**

> **DER BEFUND AUS 0.17.2 IST GEFUNDEN, NACH DREI RUNDEN — und es war die ganze
> Zeit diese eine Zeile.** In Chromium klemmt `max-height: max-content` die
> Forderung einer kurzen Liste; *deshalb war der Befund zwei Runden lang nicht
> nachstellbar, und deshalb sind zwei Runden lang Ursachen gebaut worden, die es
> nicht gab.* **In der laufenden Instanz klemmt sie nicht:** dort fordert JEDE
> Liste ihre zehn Zeilen, auch die leere. **Gemeldet mit Bild und Fingerprint
> `d3113d62`:** die Karte „Zugänge" mit fünf Zeilen stand **728** Pixel hoch, wo
> dieselbe Karte in Chromium **498** misst; der Papierkorb mit **leerer** Liste
> stand allein in seiner Reihe und war trotzdem rund **570** hoch. *Die Differenz
> ist auf den Pixel die geforderte statt der gebrauchten Höhe.*
> **DER EIGENTLICHE FEHLER IST DER UMGANG MIT DEM NICHT NACHSTELLBAREN BEFUND
> (Stolperstein 257):** die Frage hätte lauten müssen, was an der Messmaschine
> anders ist als an der Instanz — nicht, welche Ursache sich denken lässt.

> **WAS DAS KOSTET, UND DER VERMERK GEHÖRT DAZU (Stolperstein 201).** Eine Liste
> **nutzt die Höhe einer höheren Nachbarin nicht mehr aus**; die Zusage aus
> 0.17.4 ist zurückgenommen. *In einem Flexkind ist `max-height` die einzige
> Eigenschaft, die einen Deckel setzt, und sie klemmt beides — Forderung UND
> Wachstum.* **Neben „Zugang" (1055 px) zeigt die Sitzungsliste wieder zehn
> Zeilen statt zwanzig, und darunter bleibt die Kachel leer** — so leer wie
> „Darstellung" daneben, die dasselbe tut. *Gleich hohe Reihen erzeugen Leerraum
> in der kürzeren Kachel, und das war die Zusage, nicht der Fehler.*
...
> **DER SYSTEMBEREICH SIEHT ANDERS AUS ALS MIT 0.17.4, UND ER ZEIGT DASSELBE.**
> **Keine Karte steht mehr höher da, als sie Inhalt hat** — der Papierkorb mit
> leerer Liste misst 225 statt rund 570, „Zugänge" mit fünf Zeilen 498 statt
> 728. *Es geht keine einzige Angabe verloren; was vorher rollte, rollt
...
*Davor, am 31. August:* **Die Blöcke ab hier gehören 0.17.4 und den Runden
davor.**

> **DIE VORHERIGE ZEILE, ZUM VERGLEICH: 0.17.4** — Fingerprint **`d3113d62`**,
> **4811 Prüfungen**, 385 Rückbauten. *PATCH, ebenfalls keine Datenbankstufe:
> `align-items: start` zurückgenommen, die Regel „die Reihe ist so hoch wie ihre
> höchste starre Kachel" aufgeschrieben, fünfzehn Zeilen fürs
> Sicherheitsprotokoll, eine Zeile für eine leere Liste, kein Deckel im
> Fenster.* **IM FELD BESTÄTIGT AM 31. AUGUST 2026: die laufende Instanz meldete
> `d3113d62`** (Stolperstein 158) — **und genau dieser Beleg hat den Fehler
> aufgedeckt**, denn damit stand fest, dass die Bilder wirklich 0.17.4 zeigen.
> *Der Mechanismus unter der Regel ist mit 0.17.5 zurückgenommen; die Regel
> selbst und die drei Maße stehen.*
...
> **Fehler**: die Kriterienliste zerfällt bei genau einem Zugang. *Er ist
> älter als 0.16.0 und in dieser Runde nur sichtbar geworden.* Alle neun
> sind als **0.17.0** ausgearbeitet (Abschnitt 10a). **Das ist der Ertrag des Rundlaufs von Hand,
> und er ist größer als der des Fingerprints.**
>
> **ZWEI DAVON TREFFEN DIE GLOCKE SELBST**, also das Neue dieser Runde: die
> Tafel sagt nicht, WAS neu ist („7 neue Beiträge" — Kommentare? Bewertungen?),
> und sie trägt einen Erklärtext über Zeitpunkte und Lesestände, der auf eine
> Oberfläche nicht gehört. *Beides hätte kein Prüflauf je gefunden: die Zahl
...
> vermeiden:** der Browser zeichnet den Rahmen live, der Server schneidet ihn,
...
eingestellten Ausschnitt auch wirklich**, weil sie aus `medium` erzeugt wird. Alles davon ist **abgeleitet** aus `art` und `dauer`
...
| `thumb` | Kachel **512 × 512, mit eingerechnetem Ausschnitt** *(seit 0.19.5)* | dasselbe, **aus `medium` erzeugt** |
...
> entsteht `thumb` aus `data`; am Video steht dort die Videodatei, **also
...
>
> **DIE ÜBERSETZUNG DER BEIDEN ALTEN ADRESSEN IST MIT 0.19.2 ABGEBAUT.** Sie
> schickte `#/system/anlage` und `#/system/instanz` still auf `installation` —
> *für den Fall, dass ein Lesezeichen oder ein Link aus einer Mail darauf
> zeigt.* **Diesen Fall gibt es an einer Anlage mit einem Zugang nicht**, und
> eine Tafel, die ihn abfängt, will bei jeder weiteren Umbenennung gepflegt
> werden. **Beide alten Adressen fallen jetzt auf den ersten sichtbaren
> Abschnitt zurück** — derselbe Weg wie jede unbekannte Adresse.
> *Der Grund steht im Quelltext an ihrer Stelle, samt der Bauform, die es
> bräuchte, wenn der Fall je einträte: eine Tafel, einmal nachgeschlagen und
> nicht verkettet (Stolperstein 201).*
>
...
- **Der Fokuspunkt schneidet die ABLEITUNG, nie das Original.** *Bis 0.19.4
  ...
  **Seit 0.19.5 rechnet der Server den Ausschnitt in `thumb`**; die drei Werte
  ...
  jederzeit korrigierbar, und die Kachel wird aus dem Original neu erzeugt.*
...
- **Die Reihe ist so hoch wie ihre höchste starre Kachel** (seit 0.17.4, der
  ...
  **ZWEI ZEILEN TRAGEN SIE, UND KEINE HÄNGT AN EINEM SCHLÜSSELWORT** (seit
  0.17.5): `flex: 0 1 auto` — *wie hoch bin ich?* (so hoch wie mein Inhalt, und
  ich nehme mir nichts von dem, was die Kachel neben mir übrig hat) — und
  `max-height: <Deckel>rem` — *wo ist Schluss?*. Dazu `min-height: 0`, ohne die
  ein Flexkind wächst statt zu rollen (Stolperstein 237). *In `rem` und nicht in
  Pixeln: die Oberfläche stellt ihre Schrift von 80 bis 120 Prozent.*
  > **0.17.2 BIS 0.17.4 STAND HIER `flex: 1 1 <Deckel>rem` MIT
  > `max-height: max-content`, UND DAS WAR DER FEHLER** (Stolperstein 256); der
  > Vermerk bleibt stehen (Stolperstein 201). Der Gedanke war richtig:
  > `flex-basis` sollte die **Forderung** deckeln, `max-content` sie einer
  > kurzen Liste wieder wegnehmen, `flex-grow` die geschenkte Höhe einer höheren
  > Nachbarin **nutzen** lassen. *In Chromium tat es genau das.* **Nur eben
  > nicht überall:** wo `max-content` im Blockfluss die Forderung nicht klemmt,
  > fordert JEDE Liste ihre zehn Zeilen — auch die leere. **Damit fällt auch die
  > Zusage „der Deckel begrenzt die Forderung und nicht die Nutzung":** seit
  > 0.17.5 ist er **beides**, und eine Liste neben einer höheren Nachbarin
  > lässt den Rest der Kachel leer.
...
- **Das Raster des Sicherheitsprotokolls gehört der LISTE und nicht der Zeile**
  (seit 0.17.5). *Bis dahin war jede Zeile ihr eigenes Raster
  (`128px 1fr 1fr auto auto`); die beiden letzten Spalten sind `auto`, und wo
  nichts darin steht, fallen sie auf null und die beiden `1fr` teilen sich den
  Platz.* **Der Name in Spalte drei stand damit in jeder Zeile woanders** —
  gemessen in Chromium bei 1410 px vier verschiedene linke Kanten (706, 708,
  734, 769), nach dem Umbau **eine** (693).
  **Die Liste spannt die Spalten, die Zeile setzt ihre fünf Felder über
  `display: contents` direkt hinein.** Damit misst eine Spalte so breit wie ihr
  breitester Inhalt im **ganzen** Protokoll. *Feste Spalten wären der andere Weg
  und der schlechtere: „→ Gelöschter Benutzer 128" misst 176 px,
  „zusammengeführt" 110 — zusammen 286, die jede Zeile mitschleppte.*
  **ZWEI ZEILEN FOLGEN DARAUS, DIE SONST WIE KOSMETIK AUSSÄHEN.** Eine Zeile
  ohne eigenen Kasten kann keine Trennlinie tragen: die Linie sitzt an den
  Feldern, und der Spaltenabstand steht deshalb als `padding-right` am Feld und
  nicht als `column-gap` am Raster — *ein Abstand zwischen den Spalten risse die
  Linie in fünf Stücke.* Und die Felder richten sich an der **Unterkante** aus
  statt an der Schriftlinie: *`align-items: baseline` gibt einem Feld ohne Text
  keine Schriftlinie — es blieb 15 px hoch, wo seine Nachbarn 35 maßen
  (Stolperstein 258).*
  **AUF DEM SCHMALEN SCHIRM TRÄGT DIE ZEILE IHR RASTER WIEDER SELBST.** *Fünf
  ...
  sechste Feld der ersten Zeile in die Reihe der zweiten (Stolperstein 259).*
...
256. **`max-content` im Blockfluss ist ein Schlüsselwort, auf das kein Verlass
    ist.** Seit 0.17.2 stand an beiden Listen
    `flex: 1 1 <Deckel>rem; max-height: max-content` — der Deckel als
    **Forderung**, die eine kurze Liste über `max-content` wieder loswird. **In
    Chromium tut das genau, was es soll.** In der laufenden Instanz nicht: dort
    fordert JEDE Liste ihre zehn Zeilen, auch die leere, und die Karte wird so
    hoch, wie die Liste FORDERT statt wie sie BRAUCHT. *Gemessen: „Zugänge" mit
    fünf Zeilen 728 px gegen 498 in Chromium; der Papierkorb mit leerer Liste
    rund 570 gegen 225.*
    **Die allgemeine Fassung ist die wichtigere: eine Regel, die nur in dem
    Browser gemessen wird, in dem sie gebaut wurde, ist nicht belegt — sie ist
    nur nicht widerlegt.** *Was daraus folgt, ist keine zweite Messmaschine,
    sondern Zurückhaltung: wo eine einfache Bauform reicht, ist sie die
    richtige.* **`flex: 0 1 auto` mit `max-height: <Deckel>rem` sagt dasselbe
    mit Mitteln, die es seit zwanzig Jahren überall gibt** — und kostet die
    Zusage, dass eine Liste die Höhe einer höheren Nachbarin ausnutzt.

257. **Ein Befund, den man nicht nachstellen kann, ist nicht erledigt — er ist
    unerklärt, und der Unterschied zwischen Meldung und Messung IST der Fund.**
    Der Leerraum ist am 31. August zu **0.17.2** gemeldet worden. **0.17.3 hat
    eine Ursache dafür gebaut** (`align-items: start` am Kachelraster) — sie war
    falsch und hat die gleiche Höhe der Reihen zerstört. **0.17.4 hat die Zeile
    zurückgenommen, die Regel richtig aufgeschrieben und alles nachgemessen** —
    und den Befund trotzdem nicht behoben, weil der Mechanismus darunter blieb.
    *Beide Male ist gemessen worden, beide Male stimmten die Zahlen, und beide
    Male war der Befund am Bildschirm des Betreibers weiterhin da.*
    **Die Frage hätte nach dem ersten Fehlschlag lauten müssen: was ist an
    MEINER Maschine anders als an SEINER?** Nicht: welche Ursache lässt sich
    denken. *Die drei Runden haben genau eine Sache gebraucht, um zu Ende zu
    kommen — ein Bild MIT dem Fingerprint daneben, das beweist, dass beide
    dieselbe Fassung ansehen.*

258. **`align-items: baseline` gibt einem Feld OHNE Text keine Schriftlinie.**
    Es bleibt auf seiner eigenen Höhe stehen — im Sicherheitsprotokoll 15 Pixel
    gegen 35 bei den Nachbarn. *Solange die Zeile selbst der Kasten war, fiel
    das nicht auf; sobald die **Felder** die Trennlinie tragen, sitzt sie an
    genau den leeren Feldern zwanzig Pixel zu hoch und die Linie sieht aus wie
    eine Treppe.* **Wer eine Linie an Zellen hängt, richtet sie an der
    Unterkante aus und nicht an der Schrift.**

259. **Fünf Felder in zwei Spalten gehen nicht auf.** Bei `display: contents`
    hat die Zeile keinen eigenen Kasten mehr — ihre Felder liegen direkt im
    Raster der Liste. **Ändert eine Medienabfrage die Spaltenzahl, laufen die
    Felder der einen Zeile in die Reihe der nächsten**, und aus zwei Vorgängen
    wird eine Zeile. *Wer die Spaltenzahl an einem Umbruchpunkt ändert, muss die
...
272. **Ein Inline-Stil schlägt jede Regel aus dem Stilblatt — auch die, die man
    ...
    **Deshalb geht der Wert als EIGENSCHAFT hinaus (`--zoom`), und gerechnet
    ...
    > Zoom wird seither in die Kachel gerechnet; `--zoom` steht in keiner Datei
...
276. **`transform: scale()` OHNE `transform-origin` SPERRT DIE RÄNDER AUS.**
    *(Der Gegenstand ist seit 0.19.5 weg — der Ausschnitt wird eingerechnet, und
...
279. **EINE SPALTE HINTER EINEM BLOB KOSTET DEN GANZEN SATZ — ganz unabhängig
    davon, wie die Abfrage geschrieben ist.** SQLite liest einen Datensatz von
    vorn; wer Spalte 6 will, muss über die Spalten 3, 4 und 5 hinweg, und
    liegen dort Blobs, heißt das: ihre Overflow-Ketten lesen — bei SQLCipher
    zusätzlich entschlüsseln.
    **Gemessen an `photos` (312 MB, 400 Zeilen à 512 kB), je Abfrage über die
    ganze Tabelle:**
    ...
    **`MATERIALIZED` hilft dagegen NICHT** — die Zwischenabfrage muss die
    Spalte ja lesen, um sie weiterzureichen. *Das ist der Unterschied zu
    ...
    **Die Regel für den Entwurf:** was oft gefragt wird, gehört VOR die Blobs
    — oder in einen Index.

280. **EINE ZAHL, DIE AN EINEM ANDEREN GEGENSTAND GEMESSEN WURDE, IST FÜR
    DIESEN GEGENSTAND KEINE MESSUNG.** Der Auftrag zu 0.19.1 nannte „0,1 ms"
    und „0,2 ms" für zwei materialisierte Zwischenabfragen. Beide Zahlen sind
    echt — aber an einer Tabelle gemessen, in der die gelesene Spalte **vor**
    den Blobs steht. An `photos` kostet dieselbe Form **1343 ms**.
    **Die Runde hat sie ungeprüft in den Quelltext und in drei Papiere
    geschrieben**, und die Karte blieb langsam. *Das ist dieselbe Sorte Fehler
    wie die beiden, die 0.19.1 gerade gestrichen hatte — nur eine Stufe
    subtiler: die Zahl war nicht erfunden, sie war übertragen.*
    **Wo eine Zahl aus einem Auftrag, einem Papier oder einem fremden Lauf
    übernommen wird, gehört sie am eigenen Gegenstand nachgefahren** — und wenn
    das nicht geht, gehört *„übernommen von …, hier nicht nachgemessen"*
    daneben (Stolperstein 252).
    *Die billige Probe: fährt die Zahl den Fall, um den es geht? Die 0,1 ms
    sind an einer Abfrage entstanden, die `art` gar nicht las.*

281. **EIN INDEX AUF EINER NACHGERÜSTETEN SPALTE GEHÖRT HINTER IHRE MIGRATION,
    NICHT IN DIE DDL DANEBEN.** `photos.art` steht in der DDL — aber
    `CREATE TABLE IF NOT EXISTS` rührt eine vorhandene Tabelle nicht an
    (Stolperstein 13), und eine Datenbank aus 0.8.0 bis 0.8.40 trägt die Spalte
    erst, nachdem `migration0850()` gelaufen ist.
    **Ein `CREATE INDEX` im DDL-Block scheitert dort mit „no such column: art"
    — beim ÖFFNEN der Datei, also bevor der Server überhaupt startet.** *Kein
    Fehlerbild, keine halbe Funktion: die Anwendung kommt nicht hoch.*
    **Gefunden hat es der Prüfstand beim ersten Lauf**, an der nachgebauten
    Datenbank aus 0.8.40 — *die Gruppe, die es fand, gibt es seit 0.8.50 und
    sie prüft etwas ganz anderes.*
    **Die Regel gilt für jede Zeile, die eine nachgerüstete Spalte NENNT** —
    Index, Trigger, `CREATE VIEW`, erzeugte Spalte. *Die DDL beschreibt eine
    frische Datenbank; alles, was auf eine Migration wartet, steht dahinter.*
...
297. **EIN MERKMAL, DAS „SCHON BEARBEITET" VON „NOCH NICHT" TRENNT, MUSS OHNE
    ...
    zugeschnittene Kachel kleiner als die Zielkante, sobald der Ausschnitt es ist**
...
298. **EINE ANTWORT, DIE NACH DEM ANSICHTSWECHSEL EINTRIFFT, ZEICHNET IN EINE
    ...
    FEHLERMELDUNG.** `document.getElementById()` gibt dann `null`, die nächste
    Zeile wirft „can't access property …", und weil der Schreibweg seinen
    eigenen `catch` hat, **wird daraus eine ROTE Meldung über einen Vorgang,
    der geglückt ist.** *Gemeldet aus dem Feld: Ausschnitt auf 400 % stellen
    und sofort auf die Übersicht — die Wartezeit der Route (494 bis 873 ms) ist
    genau das Zeitfenster dafür.* **Die Wache gehört an die Zeichenfunktion und
    nicht an die Aufrufstelle**: sechs Rufer standen hier hinter einem `await`,
    und der siebte käme ungeschützt dazu. *Die Zusage selbst bleibt stehen —
    gespeichert ist gespeichert, und eine Bestätigung, die genau dann
    verschwiegen wird, wenn niemand hinsieht, ist keine.* **Und die Wache
...
322. **EINE ZUSAGE, DIE AN VIER FÄLLEN HÄNGT, MUSS AN DEM FALL STEHEN, DER SIE
    SEHEN KANN.** *Rückbau 646 legt den Ausschnittrahmen nach der ungerasteten
    Kante — genau das, was `setzeKiste()` verhindert. Rot wurde er nur an den
    beiden Kanten-Zusagen; „und die gegenüberliegende Ecke bleibt liegen" blieb
    grün. Der Grund: geprüft war die untere rechte Ecke, und die zieht
    `rechts-unten` — ihr Anker ist die linke obere Ecke, und `lage()` gibt dort
    `{ l: k.links, o: k.oben }` zurück; die gerastete Kante kommt darin gar
    nicht vor. An dieser einen der vier Ecken kann der Rückbau nichts
    verschieben.* **Die Zusage war richtig und stand am einzigen Fall, an dem
    sie nichts sehen konnte.** *Seit 0.22.1 steht sie zusätzlich an der oberen
    linken Ecke, deren Anker `rechts - e` und `unten - e` sind.* **Das ist der
...
|---|---|---|---|
| **Die fünf Gesten am Ausschnitt — 0.22.1** *(neu)* | — | **34** | **Zuerst die Entscheidung selbst, ohne Zeiger:** außen, innen, jede der vier Ecken, jede der vier Kanten, die Überlappung (**die Ecke gewinnt**), die Breite der Zone auf **beiden** Seiten der Grenze, der Deckel am kleinen Rahmen. **Dann am lebenden Objekt, mit echten Zeigerereignissen:** die linke obere Ecke eines neuen Rahmens sitzt, wo der Zug anfing; das Schieben rührt den Zoom nicht an (am gesendeten Rumpf); die feste Ecke und die feste Kante bleiben liegen; der Mittelpunkt wandert auf ihr nicht; nichts verlässt das Bild; ein Griff ohne Weg speichert nicht und verstellt nichts, ein Klick außerhalb speichert weiter; der Zeiger sagt es vorher; der Finger schiebt. *Davon sind **fünf** Zusagen aus der Gruppe „Fokuspunkt in der Oberfläche" herübergekommen — das Verlassen des Modus räumt jetzt fünf Zeigerbehandler ab statt drei und gehört zu dieser Frage.* |
| **Fokuspunkt in der Oberflaeche** | 34 | **29** | *Unverändert; die fünf Zusagen zum Verlassen des Modus stehen jetzt in der Gruppe darüber.* |
| **Zwei Kaesten in der Oberflaeche — 0.21.0** | 37 | **47** | Die Kopfzahl steht **genau einmal** und die Zahl steht trotzdem da; ohne Zahl bleibt der Satz, je Kasten mit eigenem Wort; der Titel sagt in **beiden** Kästen wortgleich, dass die Zahl über alle Benutzer geht, und der Erklärkasten sagt es auch; an einer Idee gibt es den Bewertungskasten **nicht**, mit vorhandenen Sternen steht er **sichtbar** da, und der Schalter „Getestet" holt ihn zurück |
| **Zwei Kaesten, zwei Durchschnitte — 0.21.0** | 33 | **40** | Die Route weist eine Bewertung am ungetesteten Eintrag ab, **nennt den Grund** und **schreibt dabei nichts**; das Potenzial geht weiter; `value: 0` geht weiter; am getesteten Eintrag nimmt sie wieder an |
| **Rechte an Testtagen und Bewertungen** | 18 | **24** | Dieselbe Absage für **Verfasser und Admin** — die Klemme kennt keine Rolle; an einer echten Datenbank mit drei Benutzern, und der Nachweis geht an den Zeilen und nicht an der Antwort |
| **zusammen** | **5661** | **5713** | **+52** |

> **BEIDE ZAHLEN SIND GEFAHREN UND NICHT GERECHNET** — der volle Lauf am
> gebauten Stand meldet **5713 von 5713**. *Die Aufteilung je Gruppe ist aus
> der Ausgabe des Laufs gezählt und gegen den Diff gehalten: **56 neue
> `pruefe()`-Aufrufe, vier weggefallene**, macht +52.*

---

### Der Stand davor: 0.22.0

**5661 von 5661 bestanden** — **90 neue, keine weggefallen.**
...
|---|---|---|---|
| **Die Ansicht kann fort sein — 0.19.6** *(neu)* | — | **9** | **Geprüft wird der echte Weg und kein nachgestelltes DOM.** Der Ruf auf `…/focus` wird **angehalten**, der Schieber geht auf **400** und speichert, **die Adresse wechselt wirklich** auf `#/`, `route()` zeichnet die Übersicht — und **erst dann** kommt die Antwort zurück. Belegt wird, dass dabei **keine rote Meldung** entsteht und **die Zusage „gespeichert" trotzdem dasteht**; dazu die **Gegenprobe an der stehenden Ansicht** (der Streifen zeichnet seine Kacheln wirklich — sonst wäre die Wache ein Ausschalter) und zwei Zusagen am Quelltext, dass **beide** Zeichenwege sie tragen. *Zwei Zeilen prüfen zuerst die Prüflage selbst — ohne sie wäre „keine rote Meldung" trivial wahr, sobald der Ausschnittmodus gar nicht aufgeht (Stolperstein 161).* |
| **zusammen** | **5237** | **5246** | **+9** |

> **DIE UMBENENNUNG HAT KEINE EINZIGE ZUSAGE GEKOSTET.** 49 Stellen im
> Prüfstand tragen das neue Wort, vier davon prüfen es *(die Zeile
> `const geschnitten` in `bilder.js`, der Aufruf `erneuereKacheln` in
> `server.js`, die Nachricht `{ art: 'erneuert' }` aus dem Thread und der Satz
> „Bildausschnitt bereits eingerechnet" in der Karte)*. **Die Zahl der
> Prüfungen ist dieselbe geblieben** — es ist dasselbe geprüft, nur anders
> benannt.

**Und die Runde davor, zum Vergleich — 5237 von 5237 bestanden** (0.19.5) —
**30 neue, keine weggefallen.**

| Gruppe (0.19.5) | vorher | nachher | wofür |
|---|---|---|---|
| **Der Ausschnitt steckt in der Kachel — 0.19.5** *(neu)* | — | **27** | **Die Vorlagen sind hier nicht einfarbig, und das ist der Unterschied zur Gruppe darunter:** wo der Ausschnitt SITZT, lässt sich an einer einfarbigen Fläche gar nicht zeigen. Jede Vorlage trägt **vier verschieden gefärbte Viertel**; welches die Kachel zeigt, sagt ihr Mittelwert. Belegt wird, dass ein frisch hochgeladenes 16:9-Foto eine **quadratische 512er Kachel** trägt und bei den Vorgabewerten die Mitte zeigt, dass die **Fassung** an der Fotozeile **und** am Hauptbild der Übersicht steht, dass das Speichern des Ausschnitts **die gewählte Ecke** in die Kachel schneidet und **die Antwort die NEUE Fassung trägt**, dass die **gegenüberliegende Ecke** überhaupt entsteht *(ohne die Klammer in `schnittRechteck()` wirft `sharp` dort)*, dass **`medium` ungeschnitten** bleibt, dass ein Ausschnitt unter der Zielkante **nicht hochgerechnet** wird und eine große Vorlage sie **auch bei `zoom` 400 erreicht**, dass der Zuschnitt **in den GEDREHTEN Maßen** rechnet *(eine Marke, die gespeichert oben links lag, findet sich nach dem Drehen oben rechts)*, dass **beide Rechnungen** — Browser und Server — über **140 Wertepaare** übereinstimmen, dass die Fälligkeitsfrage ein **Festpunkt** ist, dass der Zuschnitt **im Browser weg** ist und die Adresse die Fassung **an einer Stelle** bekommt, und dass die **Videokachel aus `medium`** eingerechnet wird, während ihr `medium` unverändert stehen bleibt |
| **Die Ableitung folgt der Anzeige — 0.19.4** | 15 | **13** | **umgeschrieben, nicht gekürzt.** Sie prüft ihre Zusagen jetzt an `makeVariants()` selbst statt über den Server — *kein Rückzug, sondern die Folge der Runde:* der Anfrageweg liefert eine zugeschnittene Kachel, und die ist immer quadratisch. **Die Zusagen gelten der UNGESCHNITTENEN Ableitung, und die muss dieselbe bleiben.** Zwei Zeilen sind dabei zusammengefallen, weil das Seitenverhältnis ohne Zuschnitt eine Rechnung ist und keine Messung |
| **Der Bestandslauf faehrt in einem eigenen Thread — 0.19.3** | 45 | **49** | **fünf neue** für die vierte Aufgabe und das Erzeugen: die einzelne Zeile wird eingerechnet und **meldet ihr Ergebnis**, eine Zeile, die es nicht gibt, meldet `ok: false` **und wirft nicht**, die Videozeile ist **nicht mehr übersprungen**, sondern **aus ihrem `medium` eingerechnet**, ihr `medium` steht **byte-genau** unverändert da, und der Server startet jetzt für **vier** Aufgaben einen Thread |
...
| **Die Bildablage in der Oberflaeche** | 39 | **40** | die Karte nennt jetzt das **Quadrat** der kleinen Ableitung und den Ausschnitt darin statt einer Kante — **und die Fortschrittszeile kennt beide Richtungen**: eingerechnet wird die Kachel in der Regel kleiner, und eine Zeile, die nur „mehr" sagen kann, zeigte in die falsche Richtung |
...
> zugeschnittene Kachel an Bytes kostet (−34,2 % über 36 Vorlagen) und was
...
> hergibt"* hielten die Zusage fest, die diese Runde zurücknimmt — **eine
> Prüfung auf eine zurückgenommene Entscheidung ist keine Lücke, sondern eine
> Falschaussage** (dieselbe Lehre wie in 0.13.2). *An ihre Stelle tritt
> „hängt an keinem Schlüsselwort mehr": sie hält fest, dass `max-content` in
> der Regel nicht mehr vorkommt.*

**Und die Runde davor, zum Vergleich — 0.17.4 brachte 6 netto:**
...
- **DIE ÜBRIGEN ZEICHENWEGE DER DETAILANSICHT TRAGEN DIE WACHE AUS 0.19.6
  ...
  Bewertungen und Testtage tun es nicht.** *Dieselbe Lücke, aber viel schwerer
  zu treffen: keiner dieser Wege wartet auf eine Bildverarbeitung — sie
  antworten in Millisekunden, und das Fenster zum Wegklicken ist entsprechend
  klein.* **Der saubere Weg wäre eine Ansichtsnummer, die `route()` hochzählt**
  und die jede Zeichenfunktion gegen ihre eigene hält. *Das ist besser als die
  jetzige Wache, weil ein fehlendes Element bei STEHENDER Ansicht weiterhin
  laut würde — heute bliebe es still.* Eine eigene Runde.
- **DIE ÜBERSICHT KANN NACH DEMSELBEN WETTLAUF EINE FASSUNG ZU ALT SEIN.** Wer
  während des Speicherns auf die Übersicht wechselt, holt die Liste, bevor die
  neue Kachel steht — sie trägt dann das `?v=` von vorher, und der Browser
  zeigt die alte Kachel aus seinem Zwischenspeicher. *Keine Fehlermeldung, kein
  Datenverlust, und beim nächsten Holen der Liste ist es weg.* **Es ist der
...
- **DIE QUADRATISCHE VORLAGE WIRD VON DER FÄLLIGKEITSFRAGE NICHT ERKANNT**
  ...
  jemand ihren Ausschnitt das nächste Mal speichert** — dann schneidet die Route
...
- **DER BEFUND AUS 0.17.2 IST MIT 0.17.5 GEFUNDEN UND BEHOBEN** — nach drei
  ...
  die Frage konnte lauten „was ist an meiner Maschine anders" statt „welche
  Ursache lässt sich denken" (Stolperstein 257).*
  **WAS OFFEN BLEIBT: welcher Browser.** Für den Bau ist das gleichgültig — die
  neue Regel hängt an keinem Schlüsselwort. *Für die Akte wäre es gut zu
  wissen, denn dann ließe sich sagen, welche weiteren Zeilen im Stilblatt dort
  anders greifen könnten.*
- **0.17.4 IST AM WIRT ANGEKOMMEN.** *Am 31. August 2026 bestätigt: die
  laufende Instanz meldete `d3113d62`.* **Die vier Handgriffe jener Runde sind
  damit teils belegt, teils überholt:** die gleiche Höhe der Reihen und die
  fünfzehn Zeilen im Protokoll stehen; **die leere Liste war weiterhin nicht
  eine Zeile hoch**, und genau das ist der Befund gewesen, aus dem 0.17.5
  geworden ist.
...
- **DER TEILEXPORT IST ZWEIMAL GEFAHREN WORDEN, UND DAS ERGEBNIS IST NICHT
  EINDEUTIG** *(Stand 30. August 2026)*. **Der erste Lauf machte Probleme, der
  zweite lief gut** — offen ist, **ob beim ersten der Weg eingehalten war**
  (Teil 1 mit „Ersetzen", alle übrigen mit „Zusammenführen"). *Ein Befund, bei
  dem der Ablauf nicht feststeht, ist kein Befund, und ein geglückter zweiter
  Lauf ist kein Freispruch für den ersten.* **Deshalb bleibt der Punkt offen
  und wird eigens noch einmal gefahren.**
  **Was dabei mitzuschreiben ist:** wie viele Teile bei 300 MB, **eine**
...
- ~~**DIE TAGS FEHLEN AM REMOTE.**~~ **ERLEDIGT DURCH ENTSCHEIDUNG,
  30. August 2026: ES WERDEN KEINE TAGS GESETZT** — weder die ausstehenden
  noch künftige. *Der Betreiber hat das ausdrücklich entschieden; die
...
- **WARUM CLAUDE SIE OHNEHIN NICHT SETZEN KÖNNTE — UND DER GRUND STAND
...
*(0.22.0 mit Fingerprint `fd292332` ist im Feld bestätigt).* *Angefasst sind
`public/app.js`, `public/style.css` und `server.js`.*

**Drei Befunde, sieben Entscheidungen (E1 bis E7, vor dem Bauen gefallen).**

**(1) Der Bildausschnitt.** Das Rechteck aus 0.22.0 legte Lage und Weite auf
**einen** Griff: Ziehen hieß immer „neu aufziehen", und wer den vorhandenen
Rahmen anfasste, warf ihn weg (Stolperstein 319). Seither entscheidet der
**Ort** der Berührung — außen aufziehen, innen schieben, an **vier Ecken und
vier Kanten** die Weite; die gegenüberliegende Ecke bzw. Kante bleibt liegen,
und bei der Kante geht die andere Achse symmetrisch um deren Mitte mit. Die
Entscheidung steht in `ausschnittGeste()` **ohne Ereignis und ohne Betrachter**
und ist deshalb ohne Zeiger prüfbar; die Rastung auf die Fünferstufen kommt
**vor** dem Legen des Rahmens, sonst wanderte der feste Anker. Der Zeiger sagt
es vorher; auf dem Finger gibt es die acht Griffe nicht (E3). *Regel G9.*

**(2) Die Kopfzahl.** Der Kopf eines zugeklappten Sternkastens trug dieselbe
Zahl zweimal — `(⌀ 2,1)` und `⌀ 2,1 gewichtet`, beide aus `item.avgRating`
(Stolperstein 318). Die Kurzfassung fällt, solange eine Kopfzahl dasteht; ohne
Zahl bleibt der Satz. **Und die Kopfzahl sagt jetzt, wessen Zahl sie ist** —
über alle Benutzer, nicht nur der eigene, im Titel und im Erklärkasten (E5).
*Regel G10.*

**(3) Vor dem Test wird nicht bewertet.** An einem ungetesteten Eintrag stand
der Bewertungskasten zugeklappt da und nahm Platz; ein Klick ließ Sterne
vergeben. Er ist jetzt **fort**, und die Route weist eine Bewertung der Phase
`nachher` dort ab — `value: 0` bleibt offen, sonst wären vorhandene Sterne
nicht mehr wegzunehmen (Stolperstein 321). **Die Ausnahme aus 0.21.0 bleibt:**
trägt ein ungetesteter Eintrag schon Bewertungssterne, steht sein Kasten
sichtbar und offen da (E6). *Regel G11.*
...
stand nach dem Eintragen der dreizehn neuen noch auf 635 — der erste
Gegenprobenlauf hat deshalb in jedem Durchgang eine falsche rote Zeile
mitgezählt und ist verworfen worden.*
...
| | Was | Warum |
|---|---|---|
| **1** | **`drawViewer()` und `drawThumbs()` fragen zuerst, ob ihre Ansicht noch steht** | Beide werden aus **sechs** Stellen hinter einem `await` gerufen. Die Route wartet auf die neue Kachel (494 bis 873 ms), und wer in dieser Zeit auf die Übersicht klickte, ließ sie in eine Seite zeichnen, die es nicht mehr gab: `getElementById` gab `null`, `box.innerHTML` warf, **und der `catch` des Speicherwegs machte daraus eine ROTE Meldung über einen geglückten Vorgang** (Stolperstein 298) |
| **2** | **Die Wache steht an der Wurzel, nicht an der Aufrufstelle** | Sechs Wachen wären sechs Gelegenheiten, die siebte zu vergessen. *Dieselbe Regel wie in `zeichneZugaenge()`, eine Stufe einfacher: dort `isConnected` an einem gehaltenen Knoten, hier das Dasein eines frisch gesuchten* |
| **3** | **Die Bestätigung bleibt stehen** | Gespeichert ist gespeichert. *Der Toast hängt am `body` und nicht an der Ansicht — er überlebt den Wechsel von selbst; nur gezeichnet wird nicht mehr.* Eine Zusage, die genau dann verschwiegen wird, wenn niemand hinsieht, ist keine |
| **4** | **Das Wort „gebacken" ist aus dem ganzen Projekt heraus** | Aus dem Feld: *„was ist das für eine Sprache?"* — **drei Ersatzwörter statt einem**, je nachdem, wovon die Rede ist: der Lauf **erneuert**, eine Ableitung wird **erzeugt**, der Ausschnitt ist **eingerechnet**, die fertige Kachel **zugeschnitten**. *Mitgezogen: fünf Bezeichner, eine Nachricht zwischen Thread und Server, sechs Suchtexte der Gegenprobe, 22 Verweise auf einen Gruppennamen und 49 Zusagen im Prüfstand* |

**Prüfstand: 5246 von 5246** — **neun neue**, keine weggefallen.
**Drei neue Rückbauten** (532 → 535, Nummern 541 bis 543), alle drei gefahren,
**keiner stumm**. **Ein neuer Stolperstein: 298.**

**Was offen blieb:** die übrigen Zeichenwege der Detailansicht (eine
Ansichtsnummer wäre der saubere Weg) und die Übersicht, die nach demselben
Wettlauf eine Fassung zu alt sein kann — beides in Abschnitt 8.

### 0.19.5 — „Der Ausschnitt wird eingerechnet, nicht gezogen"
...
| # | Was | Womit belegt |
|---|---|---|
| **1** | **Die Kachel wird eingerechnet, nicht gezogen** | `focus_x`, `focus_y` und `zoom` hören auf, eine Anweisung an den Browser zu sein, und werden das **Rezept** für `thumb`. *In Quellpunkten je Anzeigepunkt auf der 299 px breiten Kachel: bei `zoom` 235 gab der gezogene Weg **0,73×** — 1,37fach hochgezogen —, der eingerechnete gibt **5,88×**, unabhängig von der Weite des Ausschnitts.* **Das Original bleibt unangetastet, und genau deshalb bleibt der Ausschnitt jederzeit änderbar** |
| **2** | Die **Rechnung steht zweimal** und wird gegeneinander gehalten | Der Browser zeichnet den Rahmen live, der Server schneidet ihn, dazwischen liegt HTTP — eine gemeinsame Fassung gibt es nicht. **Sie steht deshalb auf jeder Seite in GENAU EINER Funktion** (`zuschnittKiste()`), und der Prüfstand hält beide über **140 Wertepaare** gegeneinander (Stolperstein 293) |
...
| **5** | Die Fälligkeitsfrage heißt **„ist die Kachel quadratisch?"** | `traegtAlteGeometrie` (lange Kante genau 400) reichte nicht mehr: eine Zeile mit `zoom = 235` und 512er kurzer Kante trägt die Geometrie aus 0.19.4 **und braucht trotzdem einen Schnitt.** *„Quadratisch **und** Zielkante" wäre der Fehler gewesen — mit `withoutEnlargement` fällt eine zugeschnittene Kachel unter 512, sobald der Ausschnitt es tut, und fiele bei jedem Start zurück in die Auswahl* (Stolperstein 297) |
| **6** | **Wo gerechnet wird, hat eine Messung entschieden** | Das Erzeugen kostet **157,3 ms im Median und 247,0 ms im 95. Perzentil**, das Zurückschreiben der Kachel noch einmal **bis zu 473,7 ms**, die ganze Route **494 bis 873 ms**. *Die Grenze des Auftrags lag bei rund 150 ms.* **Also Thread, und zwar nicht knapp** — im Haupt-Thread stünde die Event Loop fünfmal so lange wie die 133 ms, die 0.19.3 freigeräumt hat |
...
> entfernt und nicht schneidet, zeigt den Mittenschnitt.* **Beides gehört in
...
> Ausschnitt. **Mit einer zugeschnittenen Kachel zeigt er ihn mit.** *Eine Kachel,
...
| # | Was | Womit belegt |
|---|---|---|
| **1** | Die Bestandskarte fragt **wirklich** schnell | 0.19.1 hat die eine von **zwei** Ursachen behoben. Die zweite: **eine Spalte hinter einem Blob kostet den ganzen Satz** — `art` gruppiert **1338,8 ms**, aus einem Index **0,1 ms**; `MATERIALIZED` hilft dagegen nichts (Stolperstein 279). *Und `!=` schlägt den Index aus: 1334 ms gegen 0,5 ms.* **Die ganze Route: 4698 ms → 28,6 ms kalt, 4,4 ms warm** |
...
> und der Preis wäre eine zweite Wahrheit über denselben Bestand
> (Stolperstein 47): eine mitgeführte Zählertabelle müsste an jedem Weg
> nachgezogen werden, der Bilder anlegt oder entfernt, und der erste vergessene
> ließe die Karte still falsche Zahlen zeigen.*
...
**PATCH · 31. August 2026 · der Befund aus 0.17.2, gefunden nach drei Runden.**
*Gemeldet mit Bildern von der laufenden Instanz und — zum ersten Mal — **mit dem
Fingerprint dazu**: `d3113d62`, also nachweislich 0.17.4.* **Nur
`public/style.css` ist angefasst.**

**ES WAR DIE GANZE ZEIT EINE EINZIGE ZEILE: `max-height: max-content`.** Seit
0.17.2 stand an beiden Listen `flex: 1 1 <Deckel>rem` mit `max-height:
max-content` — der Deckel als **Forderung**, die eine kurze Liste über
`max-content` wieder loswird. **In Chromium tut das genau, was es soll; deshalb
war der Befund zwei Runden lang nicht nachstellbar, und deshalb sind zwei Runden
lang Ursachen gebaut worden, die es nicht gab** (Stolperstein 257). **In der
laufenden Instanz klemmt die Zeile nicht:** dort fordert JEDE Liste ihre zehn
Zeilen, auch die leere, und die Karte wird so hoch, wie die Liste **fordert**,
statt wie sie **braucht**.

**GEMESSEN, UND DIE DIFFERENZ IST AUF DEN PIXEL DIE GEFORDERTE HÖHE:** die Karte
„Zugänge" mit fünf Zeilen stand in der Instanz **728** Pixel hoch, wo dieselbe
Karte in Chromium **498** misst — 230 Pixel, und `27.95rem` minus fünf Zeilen
sind 195. Der Papierkorb mit **leerer** Liste stand allein in seiner Reihe und
war trotzdem rund **570** hoch statt **225**.

**AN IHRE STELLE TRETEN ZWEI ZEILEN OHNE SCHLÜSSELWORT.** `flex: 0 1 auto` sagt:
*so hoch wie mein Inhalt, und ich nehme mir nichts von dem, was die Kachel neben
mir übrig hat.* `max-height: <Deckel>rem` sagt: *hier ist Schluss* — zehn Zeilen
für die Bedienlisten, fünfzehn fürs Protokoll. **Nachgemessen bei 1410 Pixeln:**
Papierkorb 225, „Zugänge" 498, die Kriterienliste mit fünfzig Einträgen 419 =
zehn Zeilen und rollt, das Sicherheitsprotokoll 525 = fünfzehn Zeilen und rollt.

**DER PREIS STEHT DAZU, UND ER IST BEZAHLT.** Eine Liste **nutzt die Höhe einer
höheren Nachbarin nicht mehr aus**; die Zusage aus 0.17.4 ist zurückgenommen.
*In einem Flexkind ist `max-height` die einzige Eigenschaft, die einen Deckel
setzt, und sie klemmt beides — Forderung UND Wachstum.* **Der Leerraum unter der
Sitzungsliste ist derselbe, den die starre Karte „Darstellung" daneben schon
immer hatte:** gleich hohe Reihen erzeugen Leerraum in der kürzeren Kachel, und
das war die Zusage, nicht der Fehler. *Eine Zusage, die in einem Browser gilt
und im nächsten Leerraum erzeugt, ist keine.*

**DAS RASTER DES SICHERHEITSPROTOKOLLS GEHÖRT JETZT DER LISTE.** Als Frage
gemeldet — *„dass der Name so mal links, mal rechts verschoben ist, ist
gewollt?"* — und die Antwort war nein: bis dahin war **jede Zeile ihr eigenes
Raster**, und wo die beiden letzten `auto`-Spalten leer blieben, teilten sich
die beiden `1fr` den Platz. **Gemessen vier verschiedene linke Kanten für den
Namen — 706, 708, 734, 769 —, jetzt eine: 693.** Die Zeile setzt ihre fünf
Felder über `display: contents` direkt in das Raster der Liste; die Trennlinie
sitzt dafür an den Feldern, der Spaltenabstand steht als `padding-right` statt
als `column-gap`, und die Felder richten sich an der **Unterkante** aus — ein
Feld ohne Text hat keine Schriftlinie und blieb sonst 15 Pixel hoch, wo seine
Nachbarn 35 maßen (Stolperstein 258).

**KEINE DATENBANKSTUFE, KEIN QUELLTEXT.** Sieben markierte Blöcke,
Austauschformat **11**, `F_ROUTEN` bei **69**, achtzehn Karten in fünf
Abschnitten. **Die Deckelzahlen und die leere Liste sind nicht angefasst** —
zehn, fünfzehn, eine Zeile, wie in 0.17.4 gemessen und begründet.

### 0.17.4 — „Fordern und nutzen"

> **DER MECHANISMUS UNTER DER REGEL IST MIT 0.17.5 ZURÜCKGENOMMEN.** *Die Regel
> selbst gilt, und die drei Maße stehen.* **Was fällt, ist der Halbsatz „der
> Deckel begrenzt die Forderung und nicht die Nutzung"** — er hing an
> `max-height: max-content`, und dieses Schlüsselwort klemmt nicht überall
> (Stolperstein 256). *Der Absatz darunter bleibt im Wortlaut stehen.*
...
> gewesen.*
>
> **DAS SECHSTE RÜCKEN, am 2. September 2026, kam aus einem Gespräch über den
> Betrieb.** Die wählbare Bildablage bringt drei Verfahren zur Wahl, wo bisher
> ein Schalter stand — **das ist eine Funktion, also MINOR, also 0.21.0, und
> die Bereinigung rückt.** *Dieselbe Rechnung wie bei 0.15.0.* **„Die
...
> Migrationsblock. **Gebaut vor der Bereinigung fällt er im selben Zug mit;
> gebaut danach stünde er allein gegen eine festgeschriebene Struktur.** *Das
> ist keine Vorliebe für eine Reihenfolge, sondern die einzige harte Bindung im
> ganzen Plan.* **0.16.0 hat einen siebten dazugelegt** — die Bindung gilt für
> ihn genauso, und sie ist mit beiden Runden eingelöst.
>
> **DAS VIERTE RÜCKEN KAM WIEDER AUS DEM BETRIEB, am 30. August 2026 — und die
> Entscheidung ist dieselbe wie beim dritten.** Nach dem Einspielen von 0.16.0
> und der Bestätigung des Fingerprints `aa76c352` sind an **einem** Abend acht
...
## 10a. Was in welcher Runde steckt — die Ausarbeitung

**Diese Punkte standen bis zum 28. August 2026 im Sammelblatt und haben an
diesem Tag ihre Nummer bekommen.** Damit sind sie nach Regel 3 des Sammelblatts
von dort fortgezogen: *„Was schon eine Nummer hat, steht nicht dort, sondern im
Fahrplan."* **Ihre Ausarbeitung steht deshalb hier und nur hier.**

**Jeder Punkt trägt drei Angaben in seinem Kopf** — die Art (`Fehler` ·
`Verbesserung` · `Neue Funktion` · `Design`), die Einschätzung von Claude, und
einen Satz „Draußen üblich" dazu, wer es anderswo wie löst. *Die Einschätzung
ist eine Bemerkung und keine Entscheidung; entschieden wird durch die Nummer,
die der Punkt hier trägt.*

### Zwei Dinge, die beim Zuordnen aufgefallen sind und mitgeschrieben gehören

**ERSTENS: fast kein Punkt ist ganz zugeordnet.** Jeder trägt unter „Was gebaut
werden könnte" auch das, was **ausdrücklich nicht** gebaut werden soll — der
Strom beim Export, der getippte Name im Protokoll, die Benachrichtigungstabelle,
das Umwandeln der Bildoriginale. **Diese Teile sind mitgezogen und nicht
abgeschnitten worden**, und zwar mit Absicht: *für einen Auftrag ist die Grenze
der Runde genauso bindend wie ihr Inhalt.* Wer sie hier wegließe, baute sie beim
nächsten Anlauf.
...
**Was KEINE Nummer hat, steht weiterhin im Sammelblatt** — die drei
ausgearbeiteten Punkte „Entfällt am einzelnen Kriterium", „Zwei Einträge zu
einem machen" und „Der QR-Encoder", dazu der ganze Teil II und Teil III.
...
### 0.17.5 — GEBAUT, herausgegeben am 31. August 2026

**GEBAUT am 31. August 2026.** *Nichts aus dem Sammelblatt — zwei Befunde aus
dem Rundlauf mit 0.17.4, gemeldet mit Bildern und **mit dem Fingerprint
daneben**.* **Es gab keinen Auftrag.** **Was gebaut wurde, steht im
Änderungsprotokoll `Doku/Aenderungsprotokoll_0.17.5.md` und in Abschnitt 9.**

1. **Die Karten waren so hoch, wie ihre Liste FORDERT, statt wie sie BRAUCHT.**
   *Ursache: `max-height: max-content` klemmt die Forderung nicht überall.* An
   ihre Stelle treten `flex: 0 1 auto` und `max-height: <Deckel>rem` — zwei
   Zeilen ohne Schlüsselwort.
2. **Der Name im Sicherheitsprotokoll stand in jeder Zeile woanders.** *Ursache:
   jede Zeile war ihr eigenes Raster.* Jetzt spannt die Liste die Spalten.

**Zwei Dinge sind dabei zurückgenommen:** die Zusage aus 0.17.4, dass eine Liste
die Höhe einer höheren Nachbarin ausnutzt, und der Satz, der Deckel sei „eine
Forderung und keine Grenze". *Beide hingen an demselben Schlüsselwort.*
...
Prüfstand ändern sich nicht, nur die Oberfläche wird wählbar. **Stufe 1 geht
zuerst und allein heraus** — Auftrag `Doku/Auftrag_0.24.1.md`, vom Betreiber
am 5. September 2026 als 0.24.1 benannt; *die Nummer wird vor dem Herausgeben
am Kopf des Auftrags entschieden.*
...
*(stand als Punkt 1 im Sammelblatt)*
...
### 0.19.2 — „Was 0.19.1 nur zur Hälfte getroffen hat" · *PATCH* · **GEBAUT am 2. September 2026**

**Die Ausarbeitung steht nicht mehr hier.** Was gebaut ist, steht im
Änderungsprotokoll 0.19.2 und in Abschnitt 9.

> **AUCH DIESE RUNDE KAM NICHT AUS DEM SAMMELBLATT**, sondern aus dem Rundlauf
> mit 0.19.1 — und zwei ihrer drei Punkte sind Nacharbeit an jener Runde.
> *Es gab keinen Auftrag: die Befunde kamen mit Bildern aus dem Betrieb, und
> gemessen wurde, bevor eine Zeile fiel.*
...
### 0.19.5 — „Der Ausschnitt wird eingerechnet, nicht gezogen" · *PATCH* · **GEBAUT am 3. September 2026**
...
Funktion. Das Speichern des Ausschnitts erzeugt die Kachel neu und antwortet erst
danach; der Bestandslauf erneuert den vorhandenen Bestand mit — **Videozeilen
