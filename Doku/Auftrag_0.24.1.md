...
> **AUSGESCHRIEBEN, NICHT ABGEKÜRZT — die Schärfung des Betreibers vom
> 6. September 2026 zu F3.** *Er hat sie an zwei eigenen Beispielen gestellt:
> `anmeldung.bitteAnmeldenUmFortzufahren` ist „am Ende einfach nur Login", und
> `dialog.deinZweiterFaktorIstEingeschaltet` ist „einfach `dialog.user2faon`".*
>
> **Die Sache stimmt in beiden Fällen — der Name wird aus ihr gebildet und
> nicht aus dem Satz. Nur die Schreibweise ist entschieden worden: die Wörter
> stehen ausgeschrieben da.** Aus dem ersten wird **`login.required`**, aus dem
> zweiten **`dialog.twoFactorOn`**. *Ein eingebürgertes Fachwort darf im Namen
> stehen, wo es die Sache **ist** — `2FA`, `URL`, `ID`, `API`, `CSV`; ein Wort
> wird nie zu einem Kürzel verkürzt.* **Der Unterschied ist der zwischen einem
> Wort, das so heißt, und einem Wort, das man nachschlagen muss.**
>
> **Und der Namensraum zählt mit.** Er trägt den Bereich; der Namensteil trägt
> nur noch das, was ihn von seinen Nachbarn unterscheidet. *Deshalb heißt der
> erste `login.required` und nicht `login.login`: was schon links vom Punkt
> steht, wird rechts davon nicht wiederholt.*
