...
| Deutsche Bezeichner im Code | **116, alle benannt** | **12** — sechs falsche Freunde und sechs alte Feldnamen. *Nachgemessen beim Bauen: die Vorhersage „6" hat die sechs alten Feldnamen vergessen, die `SHAPES_0242` aus 0.24.2 NENNEN muss, um sie zu übersetzen. Sie sind keine Benennung dieser Fassung, aber sie stehen im Code* |
| `WAITING_FOR_STAGE_TWO` | **104** | **fällt weg** *(F7)* |
| `OLD_STORED_NAMES` | **6** | **6, unverändert.** *Nachgemessen beim Bauen: die Vorhersage „mehr" ist nicht eingetreten, und der Grund ist eine Entscheidung im Bauen — die Tafel der neuen Migration steht als **Zeichenfolgen-Paare** (`['seite', 'side']`) und nicht als Eigenschaftsnamen (`{ seite: 'side' }`). Eine Übersetzungstafel muss nennen dürfen, was sie übersetzt, ohne es zu HEISSEN* |
...
| Datei | was |
|---|---|
| `public/languages/en.json` | **neu** — **1204 Schlüssel.** *Nachgemessen beim Bauen: die Vorhersage „1259" war zu hoch — sie zählte die Mehrzahlformen doppelt.* |
...
| `README.md` | *beim Bauen dazugekommen und im Auftrag nicht vorgesehen:* ein Abschnitt **„Sprache“** — die Wahl je Zugang, der Vorrat, und wie man eine eigene Sprachdatei dazulegt. *Der Abschnitt „Vokabular“ sagt jetzt, dass es die vierzehn Wörter je Sprache gibt.* **Ein Betreiber, der die Anleitung liest, muss von einer sichtbaren Funktion darin erfahren** |
