...
| **0.36.0** | **Sicherheit** | Fünf Lücken aus der Durchsicht vom 15. September 2026: CSRF-Token, Anmeldesperre übersteht keinen Neustart, 199 `innerHTML`-Stellen ungeprüft, Abhängigkeiten ohne Automatik. **Nach 0.35.0, damit die Durchsicht den Code sieht, der bleibt.** **Die fünf gemessenen Befunde aus der Messung zu 0.35.0 liegen nicht mehr hier:** *zwei sind mit 0.34.4 behoben, die drei übrigen stehen als **0.35.1** darüber* **DER AUFTRAG STEHT ALS `Doku/Auftrag_0.36.0.md`**, geschrieben am 19. September 2026, mit sechs Bauabschnitten und einer Fragetafel. **Die Zahlen der Durchsicht sind dort nachgemessen:** *CSRF weiterhin null Fundstellen; die Anmeldesperre liegt in `auth.js`:438 als `Map` im Arbeitsspeicher; aus 199 `innerHTML`-Stellen sind **178** geworden, davon tragen 113 nachweislich nichts von aussen — **59 bleiben zu messen**. Der vierte Punkt hat sich von selbst erledigt: `npm audit` meldet **0 Luecken**, `express@4.22.3` laesst `qs@6.16.0` zu. **Punkt 21 des Sammelblatts ist damit zu**, ohne den Sprung auf `express` 5. Was fehlt, ist die Automatik — eine Stelle, die den Lauf rot faerbt, wenn `npm audit` etwas meldet.* | offen | — |
| **0.37.0** | **Die Kommentare werden verdichtet** | Vorgabe des Betreibers vom 19. September 2026. **Drei Dinge an denselben Dateien: die Kommentare verdichten, jede Versionsnummer heraus, jeden Verweis auf ein Papier heraus.** *Gemessen am 19. September 2026:* **4.294 Kommentarzeilen in den vierzehn ausgelieferten Modulen** (22 %), dazu **1.574 Zeilen in 409 Bloecken in `public/style.css`** (54,5 % der Bytes). *`server.js` und `public/app.js` tragen zusammen 79 Prozent.* **778 Versionsnummern** — 726 davon in Kommentaren, **0 im Code**, also ohne Bezug zum Verhalten. **188 Verweise auf Papiere ueber den Namen** („Befund" 87, „Bauabschnitt" 70, der Rest 31); ueber den Pfad `Doku/` sind es **0** seit 0.35.2. **Die Regeln stehen seit dem 19. September 2026 geschaerft in `CLAUDE.md`, Abschnitt 2** — *hoechstens drei Zeilen je Block (96 Prozent der 2.716 Bloecke halten das schon), die Sache statt des Weges zu ihr, keine Versionsnummer, kein Papierverweis.* **Sie binden jede Runde und gelten, bis der Betreiber sie selbst aendert.** **Die schaerfste Zusage der Runde:** *die sechs Gleichlaufsummen bleiben gleich — sie lesen den Quelltext ohne Kommentare, und wenn sie sich nicht bewegen, ist nur Kommentar gefallen.* **Dazu die README auf Sinn lesen** *(BA 6)* **und ein dritter Waechter** *(BA 7)*, der den Papierverweis ueber den Namen faengt; die beiden alten Latten fallen auf null. **Nach 0.36.0 und nicht davor:** *jene Runde fasst dieselben zwei Dateien an, und zwei Runden auf denselben 15.000 Zeilen kollidieren in jedem Diff.* **Der Auftrag steht als `Doku/Auftrag_0.37.0.md`**, mit acht Bauabschnitten, einer Fragetafel und vier verworfenen Wegen | offen | — |
| **0.38.0** | **Ein Schreibeditor für Kommentare und Beschreibung** | Vorgabe des Betreibers vom 17. September 2026, **nach 0.37.0** — *am 19. September 2026 von 0.37.0 auf 0.38.0 gerückt, weil der Betreiber die Kommentarrunde davor gesetzt hat*. **Vier Funktionen: fett, kursiv, Liste, Zitat** — *dazu „mit Zitat antworten" im Kommentarbereich.* **Beides trägt es: Kommentare und Beschreibung** — *die Beschreibung hat heute nicht einmal eine Leseansicht, sie steht als Text im Feld.* **Die offene Frage ist selbst bauen oder fertig nehmen.** *Gemessen am 17. September 2026, gzipped: Quill 62.732 B, Trix 56.150 B, EasyMDE 109.826 B, `marked` allein 13.891 B. Die ganze Auslieferung von Kriterion ist seit 0.35.0 268.441 B — Quill wäre ein Viertel mehr, `marked` fünf Prozent.* **Offen ist außerdem, was gespeichert wird** *(Markup oder Auszeichnungstext), wie die 199 `innerHTML`-Stellen aus 0.36.0 dazu stehen, und was mit den Kommentaren geschieht, die heute schon da sind.* Der Punkt steht samt Zahlentafel als **Punkt 44** im Sammelblatt | offen | — |
