...
| **0.35.1** | **Drei Sicherheitsbefunde aus der Messung** | Die drei Befunde der Messung zur 0.35.0, die dort nicht gebaut worden sind. Zuschnitt wie 0.34.4: wenige Zeilen je Stelle, je ein Rückbau, kurzes Änderungsprotokoll. **1. `POST /api/trash/:id/restore` (`server.js`:4918) hat dasselbe Wettlaufmuster wie der Link, den 0.34.4 behoben hat.** *Zwischen `SELECT * FROM trash` und `DELETE FROM trash` liegt `await importInto(...)`; zwei gleichzeitige Anfragen sehen beide dieselbe Zeile und spielen den Eintrag zweimal ein.* **Anders als beim Link ist es kein Einzeiler:** *`importInto` ist asynchron und kann nicht in einer `db.transaction` stehen. Zwei Wege stehen offen — ein Merkmal an der Zeile, das die Datenbank entscheiden lässt (Schemaänderung), oder eine Liste der gerade laufenden Nummern im Prozess (ohne Schema, überlebt keinen Neustart — bei diesem Vorgang genügt das, weil ein Neustart mitten im Einspielen ohnehin ein anderer Fall ist).* **2. `keys.js`:88 liest die Schlüsseldatei ungeprüft**, *obwohl der Kommentar in Zeile 9 sagt, die Prüfung auf 64 Hex-Zeichen stehe an einer Stelle für alle drei Fragen. `HEX_PATTERN` greift an drei Stellen und an dieser nicht: eine abgeschnittene oder beschädigte `data/encryption.key` geht unbesehen an SQLCipher.* **Vier Zeilen.** **3. `PUT /api/settings` (`server.js`:1723) sagt ab, nachdem es schon geschrieben hat.** *Gemessen: **232 Zeilen, 13 Absagen, 14 Schreibstellen, keine Transaktion.** Ein Rumpf mit `{font: 80, strip: 999}` schreibt `font` und antwortet dann mit 400. 0.35.0 hat zwei Stellen darin vorgezogen (`filters`/`views` und die Aufräumregel prüfen vor dem Schreiben); die übrigen elf stehen noch.* **Der Weg: erst alles annehmen und prüfen, dann in einer `db.transaction` schreiben** — *dieselbe Bauform wie beim Fotoweg in 0.35.0.* **Der Fingerprint ändert sich** *(`server.js`, `keys.js`)* | nein | — |
