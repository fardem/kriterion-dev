...
| **0.35.0** | **Code-Effizienz** | **Auftrag geschrieben am 16. September 2026** — `Doku/Auftrag_0.35.0.md`, gemessen auf dem Stand `5139e1f`. **101 Befunde von 17 Lesern, 75 halten der Widerlegung stand, 5 davon sind Sicherheitsbefunde und gehen nach 0.36.0. Für die Runde bleiben 70 Befunde an 65 Stellen** — tot 24, langsam 21, umständlich 15, besser 5. *Der Betreiber hat die Runde in zwei Sätzen bestimmt: Code, der statt kompliziert und schwerfällig einfacher zu bauen wäre, und überall da, wo es hätte besser gemacht werden können — auch langsam.* **Die schwersten Einzelbefunde:** `public/style.css` ist zu **70,5 %** Kommentar *(212.626 von 301.395 Bytes, der Server komprimiert nicht — jeder Browser lädt 208 kB Kommentar je Aufruf; die Datei stand in keiner Messung von 0.34.1, weil `tools/comments.js` nur JavaScript zählt)*; `importInto` mit **372 Zeilen** und Einrückungstiefe 14; der Export steht **dreimal**, der Import **viermal** gleichzeitig im Arbeitsspeicher; `GET /api/items/:id/export` hat **kein Bedienelement**, obwohl das Handbuch die Funktion beschreibt; **626 feste Wartezeiten** in den Prüfmodulen. **Dreizehn Fragen beantwortet** — *F13 am 16. September 2026: die beiden schwersten Sicherheitsbefunde gehen als Patch 0.34.4 vor diese Runde, die übrigen drei nach 0.36.0*. **Kein Verhalten ändert sich, keine Schemaänderung, keine Quote** — je Bauabschnitt ein Rückbau auf die alte Form als Beleg, und jede Laufzeitzahl aus fünf Läufen im Median, weil die teuerste Prüfgruppe ohne jede Änderung zwischen **19,2 und 35,8 s** schwankt | nein | — |
| **0.36.0** | **Sicherheit** | Fünf Lücken aus der Durchsicht vom 15. September 2026: CSRF-Token, Anmeldesperre übersteht keinen Neustart, 199 `innerHTML`-Stellen ungeprüft, Abhängigkeiten ohne Automatik. **Nach 0.35.0, damit die Durchsicht den Code sieht, der bleibt.** **Dazu fünf gemessene Befunde aus der Messung zu 0.35.0** *(16. September 2026)*: **ein einmaliger Link lässt sich zweimal einlösen** — `auth.js:691`, zwischen der Prüfung und dem Schreiben liegt die scrypt-Rechnung, **auf einer Kopie der Datenbank nachgestellt: zwei gleichzeitige Aufrufe gelingen beide**; dasselbe Muster beim Wiederherstellen aus dem Papierkorb (`server.js:4780`); die Schlüsseldatei wird ungeprüft gelesen, obwohl der Kommentar daneben das Gegenteil zusagt (`keys.js:86`); `PUT /api/settings` sagt neunmal ab, nachdem es schon geschrieben hat, ohne Transaktion (`server.js:1655`); **ein Prüfmodul, das nach dem Schreiben seiner Meldung abstürzt, zählt als bestanden** (`testbench.js:25`). *Die beiden letztgenannten sind je wenige Zeilen; ob sie als Patch 0.34.4 vorgezogen werden, steht als F13 im Auftrag 0.35.0 offen* | offen | — |
