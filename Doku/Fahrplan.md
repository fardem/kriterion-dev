...
| **0.38.0** | **Ein Schreibeditor für Kommentare und Beschreibung** | Vorgabe des Betreibers vom 17. September 2026, **nach 0.37.0** — *am 19. September 2026 von 0.37.0 auf 0.38.0 gerückt, weil der Betreiber die Kommentarrunde davor gesetzt hat*. **Vier Funktionen: fett, kursiv, Liste, Zitat** — *dazu „mit Zitat antworten" im Kommentarbereich.* **Beides trägt es: Kommentare und Beschreibung** — *die Beschreibung hat heute nicht einmal eine Leseansicht, sie steht als Text im Feld.* **Die offene Frage ist selbst bauen oder fertig nehmen.** *Gemessen am 17. September 2026, gzipped: Quill 62.732 B, Trix 56.150 B, EasyMDE 109.826 B, `marked` allein 13.891 B. Die ganze Auslieferung von Kriterion ist seit 0.35.0 268.441 B — Quill wäre ein Viertel mehr, `marked` fünf Prozent.* **Offen ist außerdem, was gespeichert wird** *(Markup oder Auszeichnungstext), wie die 199 `innerHTML`-Stellen aus 0.36.0 dazu stehen, und was mit den Kommentaren geschieht, die heute schon da sind.* Der Punkt steht samt Zahlentafel als **Punkt 44** im Sammelblatt. **Der Auftrag steht als `Doku/Auftrag_0.38.0.md` und läuft durch** — *zwölf Bauabschnitte, einundzwanzig Fragen, alle vor der ersten Zeile entschieden, zwölf verworfene Wege.* **Er wertet einen Vorbau aus, den der Betreiber mit Google Gemini auf dem Stand 0.29.0 gemacht hat:** *neun Funktionen daraus werden übernommen, zwanzig Befunde und sechs fremde Eingriffe stehen dagegen — der erste davon schaltet die Datenbankverschlüsselung ab.* **Die Auszeichnung ist eine Teilmenge von CommonMark, und innerhalb der Teilmenge gilt die Spezifikation:** *Kriterion zeichnet, was CommonMark zeichnet, oder gewöhnlichen Text — nie etwas Drittes.* **Kursiv trägt den Unterstrich und nicht den Stern**, weil die Spezifikation dort die Auszeichnung mitten im Wort verbietet und `3*4 und 5*6` damit Text bleibt. *Acht Bauformen: fett, kursiv, Link, Zitat, Aufzählung, Nummerierung, Code in Backticks, Escape.* **Tabellen, Abhaklisten und Codeblöcke bekommen eigene Runden; Bilder nie** — *`img-src` verbietet die fremde Adresse.* **Es wird kein fertiger Editor genommen und auch keiner gebaut:** *das Textfeld bleibt, gebaut werden ein Leser (~280 Zeilen) und ein Menü (~180), und dazwischen liegt nichts.* *Eine einzige benannte Abweichung: der einzelne Zeilenumbruch bleibt ein Umbruch, weil `.cmt-body` `white-space: pre-wrap` trägt.* **Keine Schemaänderung; die Kommentarnummer zählt nach zeitlicher Reihenfolge und nicht nach der Anzeige; eine neue Route `GET /api/comment-refs`; Formatnummer 17 → 18.** *Gemessen auf 0.37.0:* **~1.500 ausgelieferte Zeilen und +22,3 KB gzip auf 253.736** — *Quill wäre 24,7 Prozent, `marked` allein 5,5 für ein Drittel der Arbeit.* **Die Doppelung in der README wird in BA 11 mit aufgelöst** *(Entscheidung des Betreibers vom 19. September 2026)* | offen | — |
...
| *ohne Nummer* | **„Auffangnetz" und „Grundausstattung" umbenennen** | **Befund aus dem Bau der 0.37.0.** *`CLAUDE.md` führt beide Wörter unter „nicht verwenden".* **Sie stehen als Abschnittsüberschrift in `db.js` und im Namen von Dutzenden Prüfgruppen und Rückbauten.** *Sie in einer Runde zu ersetzen, die etwas anderes tut, hieße den Prüfstand umzubenennen — deshalb eine eigene Runde.* **0.38.0 fasst sie ausdrücklich nicht an** *(Entscheidung des Betreibers vom 19. September 2026)* | offen | — |
