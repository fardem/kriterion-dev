...
| **0.35.2** | **Der Einzelexport wird ausgebaut** | Vorgabe des Betreibers vom 17. September 2026. **`GET /api/items/:id/export` liefert einen Eintrag als Datei — das Projekt hat dafür keine Verwendung.** *Die Route ist mit **0.8.70** abgefallen, als `entryAsBundle()` und `exportEnvelope()` getrennt wurden; sie stand in keinem Auftrag. Danach hatte sie 26 Runden lang keinen Rufer in der Oberfläche. Die Messung zur 0.35.0 fand sie als Stelle ohne Bedienelement — zwei Wege standen offen, Knopf dazu oder Route raus, gebaut wurde der Knopf.* **Ausgeliefert fallen:** *die Route (`server.js`:4372), der Knopf `#exp1` samt Rufer (`public/app.js`:3890 und 5725) und `entry.exportOne` in allen drei Sprachdateien.* **Im Prüfstand:** *`F_ROUTES` 73 → 72, `LANG_KEY_COUNT` 1.212 → 1.211, die Zahl 1.300 → 1.299, vier Sprachtafeln in `test/release_031.js`, die Rückbauten **1087 und 1088** samt ihrer Gruppe, die sechs Gleichlaufsummen.* **Der Kommentar in `public/app.js`:3885 geht mit** — *er nennt „seit 0.30.0" und damit eine falsche Versionsnummer.* **Zwei Fragen vorher:** *geht die Route mit oder nur der Knopf (Vorschlag: beides), und ist ein Routenausbau eine PATCH-Runde? 0.35.0 hat `GET /api/health` ausgebaut und war MINOR, mit Kasten im CHANGELOG.* Der Punkt steht als **Punkt 45** im Sammelblatt. **Der Fingerprint ändert sich** | offen | — |
