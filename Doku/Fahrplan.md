...
| **0.35.2** | **Der Einzelexport wird ausgebaut, Route und Knopf** | Vorgabe des Betreibers vom 17. September 2026. **`GET /api/items/:id/export` liefert einen Eintrag als Datei — das Projekt hat dafür keine Verwendung.** **Die Route geht mit**, nachdem nachgesehen ist, dass sie für nichts anderes gebraucht wird: *`entryAsBundle()` und `exportEnvelope()` bleiben — sie tragen den vollen Export und den Papierkorb; die Route selbst hat nur den einen Rufer.* *Die Route ist mit **0.8.70** abgefallen, als jene beiden getrennt wurden; sie stand in keinem Auftrag und hatte 26 Runden lang keinen Rufer in der Oberfläche.* **Ausgeliefert fallen:** *die Route (`server.js`:4372), der Knopf `#exp1` samt Rufer (`public/app.js`:3890 und 5725) und **zwei** Sprachschlüssel in allen drei Dateien — `entry.exportOne` und `server.entryTooBig`, dessen einzige Fundstelle die Route ist.* **Im Prüfstand:** *`F_ROUTES` 73 → 72, `LANG_KEY_COUNT` 1.212 → **1.210**, die Zahl 1.300 → **1.298**, die Rückbauten **1087 und 1088** samt ihrer Gruppe, die sechs Gleichlaufsummen.* **Die beiden Schlüssel fallen verschieden:** *`entry.exportOne` steht in keinem Vergleichsstand und verlässt nur die Zugangslisten; `server.entryTooBig` steht in beiden und muss namentlich in `EG_GONE_AFTER_0312`, `TR_GONE_AFTER_0313` und eine neue `WORDING_GONE`-Liste, sonst kippt die Wortlautprobe über 1.082 Sätze.* **Was danach tot daliegt:** *`exchangeParts()` und `exchangeEnvelopeBytes()` bekommen `itemId` dann nur noch als `null` — je ein `onlyOne`, `values`, `and()`/`wo()` und **11 Einsetzungen** in den Abfragen. Ob das in derselben Runde fällt, ist zu entscheiden.* **Der Kommentar in `public/app.js`:3885 geht mit** — *er nennt „seit 0.30.0" und damit eine falsche Versionsnummer.* **Eine Frage bleibt vorher:** *ist ein Routenausbau eine PATCH-Runde? 0.35.0 hat `GET /api/health` ausgebaut und war MINOR, mit Kasten im CHANGELOG.* **MIT IN DIE RUNDE, weil sie dieselben Dateien anfasst:** *(a) **ein Wächter „jede Route hat einen Rufer"** — gemessen am 17. September 2026: **73 Routen, 0 ohne Rufer**, sechs davon über die Tafel in `adminList` als `${url}/${entry.id}` (`/api/product-categories`, `/api/tags`, `/api/criteria`, je PUT und DELETE), also eine benannte Ausnahmeliste mit drei Adressen. **Das ist die Lücke, durch die der Einzelexport 26 Runden gefallen ist.** (b) **`public/style.css`:161** — der letzte Verweis auf `Doku/` in einer ausgelieferten Datei, liegengeblieben, weil eine Einzelbehebung den Fingerprint einer veröffentlichten Version verschoben hätte; diese Runde ändert ihn ohnehin.* **ZU KLÄREN, bevor gebaut wird:** *(c) **Punkt 35** — Gegenprobe 330 reißt `public/app.js` ab, statt eine Prüfung rot zu machen; laut Sammelblatt gehört sie der Runde, die `counterproof.js` anfasst, und das ist diese. (d) **Punkt 34** — der Grund eines gescheiterten Versands steht in der Sprache des Empfängers. `e.reason` entsteht in `mail.js` als fertiger Satz und müsste erst als Schlüssel reisen; **ob das noch klein ist, ist die Frage.*** *(e) **Punkt 36**, der türkische Genitiv: die beiden Stellen sind am 17. September 2026 gegen ihre vier Nachbarn gehalten worden. **Der Vorschlag ist, sie zu lassen** — die Belege stehen im Sammelblatt, die Entscheidung liegt beim Betreiber.* **Der Auftrag steht als `Doku/Auftrag_0.35.2.md`**, mit **zehn Bauabschnitten**, einer Fragetafel, einem geschlossenen Punkt (36) und sieben verworfenen Wegen. **ZWEI DAVON SIND AM 18. SEPTEMBER 2026 AUS DEM BETRIEB DAZUGEKOMMEN, und damit ist die Runde keine reine Aufräumrunde mehr:** **BA 4 — mehr als 40 Fotos auf einmal verlieren alle Fotos.** *`upload.array('photos', 40)`; beim 41. Bild bricht Multer die ganze Anfrage ab, es wird keines gespeichert, und am Bildschirm steht „Unexpected field" — der Fehler-Handler fällt für einen Fehler ohne `key` auf `err.message` zurück. Der Browser prüft beim Fotoweg weder Zahl noch Größe; ein Foto über 30 MB gibt „File too large". Eine Obergrenze je Eintrag gibt es nicht — der Browser bündelt also, statt abzusagen.* **Punkt 46 im Sammelblatt.** **BA 5 — das Containerprotokoll bekommt Zeitstempel.** *51 Zeilen mit `[Kriterion]` in sechs Dateien tragen heute keine Zeit. ISO 8601 mit Versatz, der Versatz aus `TZ`, `TZ=Europe/Berlin` in `docker-compose.example.yml`. Die gespeicherten Zeiten bleiben UTC.* **Punkt 47 im Sammelblatt.** **UND VIER OFFENE PUNKTE RÄUMEN MIT AUF — am 19. September 2026 dazugenommen, alle vier an Dateien, die die Runde ohnehin anfasst:** **BA 6, Punkt 33** — *die Startzeile schreibt `server.backupDirNotSet` roh ins Protokoll; BA 5 fasst jede `[Kriterion]`-Zeile an, also auch diese.* **BA 7, Punkt 35** — *Gegenprobe 330 reißt `public/app.js` ab, statt eine Prüfung rot zu machen; die Runde fässt `counterproof.js` ohnehin an.* **BA 8, Punkt 25** — *sechs deutsche `id`, die die Gestaltprobe nicht sieht, weil sie über `.id = '…'` gesetzt werden. Nachgemessen: sechs statt sieben, je eine Fundstelle — sechs einzeilige Umbenennungen und drei Zeilen im Wächter.* **BA 9, Punkt 43 Schritt 1** — *die zwei Löcher des 0.34.3-Sweeps, gemessen **15 Verweise** (`public/style.css` 8, `db.js` 7 im SQL-Text).* **Drei der vier sind Lücken in Wächtern** — dieselbe Art Arbeit wie BA 2. Der Punkt steht als **Punkt 45** im Sammelblatt. **Der Fingerprint ändert sich** | offen | — |
| **0.36.0** | **Sicherheit** | Fünf Lücken aus der Durchsicht vom 15. September 2026: CSRF-Token, Anmeldesperre übersteht keinen Neustart, 199 `innerHTML`-Stellen ungeprüft, Abhängigkeiten ohne Automatik. **Nach 0.35.0, damit die Durchsicht den Code sieht, der bleibt.** **Die fünf gemessenen Befunde aus der Messung zu 0.35.0 liegen nicht mehr hier:** *zwei sind mit 0.34.4 behoben, die drei übrigen stehen als **0.35.1** darüber* | offen | — |
| **0.37.0** | **Ein Schreibeditor für Kommentare und Beschreibung** | Vorgabe des Betreibers vom 17. September 2026, **nach 0.36.0**. **Vier Funktionen: fett, kursiv, Liste, Zitat** — *dazu „mit Zitat antworten" im Kommentarbereich.* **Beides trägt es: Kommentare und Beschreibung** — *die Beschreibung hat heute nicht einmal eine Leseansicht, sie steht als Text im Feld.* **Die offene Frage ist selbst bauen oder fertig nehmen.** *Gemessen am 17. September 2026, gzipped: Quill 62.732 B, Trix 56.150 B, EasyMDE 109.826 B, `marked` allein 13.891 B. Die ganze Auslieferung von Kriterion ist seit 0.35.0 268.441 B — Quill wäre ein Viertel mehr, `marked` fünf Prozent.* **Offen ist außerdem, was gespeichert wird** *(Markup oder Auszeichnungstext), wie die 199 `innerHTML`-Stellen aus 0.36.0 dazu stehen, und was mit den Kommentaren geschieht, die heute schon da sind.* Der Punkt steht samt Zahlentafel als **Punkt 44** im Sammelblatt | offen | — |
...
| *ohne Nummer* | **`public/style.css` noch einmal ansehen** | Vorgabe des Betreibers vom 17. September 2026 zu **Punkt 42** des Sammelblatts. 0.35.0 hat die Kommentare der Datei gekürzt, **der Anteil liegt danach bei 54,5 %** — *mehr als die Hälfte der Datei.* Die Datei gehört zu den achtzehn des Fingerprints, eine Runde daran ändert ihn | nein | — |
| *ohne Nummer* | **Drei kleine Punkte, die 0.35.2 ausdrücklich nicht mitnimmt** | *Aus der Durchsicht vom 17. September 2026; sie stehen in `Doku/Auftrag_0.35.2.md` als V3 bis V5 und bleiben im Sammelblatt offen.* **Punkt 33** — *die Startzeile schreibt `server.backupDirNotSet` roh ins Containerprotokoll statt eines Satzes; gehört der Runde, die `backupState()` anfasst.* **Punkt 38** — *die Kachel zählt in Worten, der Blockkopf seit 0.32.1 in Zeichen; erst messen, dann entscheiden.* **Punkt 27** — *der Aufräumer des Prüfstands flackert auf mehreren Spuren; vier Zählungen stehen jetzt da (9/21 auf vier Spuren, 2/3 und 6/12 auf dreien, 1/4 auf zweien), und `testbench.js` ist die einzige Datei.* **Keiner der drei hängt an einem anderen** | offen | — |
