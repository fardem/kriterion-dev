...
| **0.37.0** | **Die Kommentare werden verdichtet** | Vorgabe des Betreibers vom 19. September 2026, **am 19. September 2026 auf dem gebauten Stand 0.36.0 nachgemessen**. **Drei Dinge an denselben Dateien: die Kommentare verdichten, jede Versionsnummer heraus, jeden Verweis auf ein Papier heraus.** *Gemessen auf 0.36.0:* **4.328 Kommentarzeilen in den vierzehn ausgelieferten Modulen** (22 %), dazu **1.574 Zeilen in 409 Bloecken in `public/style.css`** (54,5 % der Bytes) **und 313 Zeilen im `SCHEMA`-String von `db.js`, die kein Werkzeug misst** — *`tools/segments.js` fuehrt sie als Text; `db.js` traegt in Wahrheit 45 Prozent Kommentar statt 21.* *`server.js` und `public/app.js` tragen zusammen 78 Prozent.* **778 Treffer des Nummernmusters** — 726 in Kommentaren, **44 in Strings**, 8 in der Anleitung, **0 im Code**; **15 davon sind ueberhaupt keine Fassungsnummern** (zehn Daten, zwei SVG-Pfaddaten, ein `1.0.0`, eine Adresse, ein Beispieldatum), **echte sind es 763**. **187 Verweise auf Papiere ueber den Namen** *(188 auf 0.35.2)* **und 94 Abkuerzungen dazu** — `(F<Zahl>)` 52, `BA <Zahl>` 39, `Punkt <Zahl>` 3, *die im Auftrag zuerst fehlten*; ueber den Pfad `Doku/` sind es **0** seit 0.35.2. **Die Regeln stehen geschaerft in `CLAUDE.md`, Abschnitt 2**, und binden jede Runde. **DREI ZUSAGEN DES ERSTEN AUFTRAGS TRUGEN NICHT:** *Zusage 5 — `tools/gleichlaut.js` liest nur `public/app.js`, und ihre sechs Summen aendern sich sehr wohl, wenn ein nachgestellter `//`-Kommentar faellt (der Filter streicht nur Kommentare am Zeilenanfang; nachgemessen). Zusage 4 — die Latte der Versionsnummern kennt `README.md` und `manual-de.md` nicht, und `test/source.js`:915–923 verlangt dort sechs Nummern ausdruecklich. F5 und V3 — `tools/publish.js` laesst nur `Doku`, `CLAUDE.md` und zwei Werkzeuge weg: **68 Dateien gehen hinaus, 21 davon sind der Pruefstand.*** **Der Auftrag steht als `Doku/Auftrag_0.37.0.md`**, mit **zehn** Bauabschnitten, **elf** Fragen und vier verworfenen Wegen. **Neu darin:** *BA 8 fuer die vier ausgelieferten Dateien ohne Bauabschnitt (`public/index.html`, `.env.example`, `docker-compose.example.yml`, `Dockerfile`), BA 9 fuer die acht Stellen im Pruefstand, die die Runde nachziehen muss — 15 Rueckbauten, vier Kommentarzitate, `COMMENT_ROWS`, `VN_TOTAL`, `VN_CEILING`, die Rueckbauzahl, die sechs Gleichlaufsummen und `driverScore` —, die erweiterte Wortliste in BA 7 und Punkt 43 in BA 10, der nur schliesst, wenn `tools/comments.js` das Stilblatt mitzaehlt.* **Nach 0.36.0, und jene Runde ist gebaut** | offen | — |
