...
| **F3** | **Was, wenn sich der Bestand ändert, während man blättert?** *Ein gelöschter Eintrag, ein neuer Testtag, eine geänderte Sortierung* | **Nichts.** *Die Reihenfolge ist die des letzten Zeichnens der Übersicht und wird nicht nachgezogen.* **Führt ein Pfeil auf einen Eintrag, den es nicht mehr gibt, sagt die Ansicht das** — dieselbe Antwort wie heute beim Direkteinstieg auf eine tote Nummer | |
...
| **BA 2** | **Das Blättern im Eintrag** — zwei Pfeile in der Kopfzeile, `Bild auf`/`Bild ab` auf der Tastatur, die Reihenfolge aus `state.items` | *nichts fällt. **Die Pfeiltasten bleiben bei den Bildern** — das ist die Entscheidung vom 8. September und keine Frage dieser Runde* |
| **BA 3** | **Die Behälterabfrage** für die Protokollzeile *(F4)* — das erste `@container` im Stilblatt, mit dem Satz, warum es hier steht | *keine Regel fällt; eine Fensterabfrage wird zur Behälterabfrage. **Fällt doch eine, steht sie NAMENTLICH im Protokoll*** |
...
| **BA 6** | **Das Startbildzeichen** — `GET /api/manifest.json` am Server *(F8, F9)*, `<link rel="manifest">` in `index.html` | *nichts fällt. **`F_ROUTES` geht von 72 auf 73**, und das gehört als einzige Zahl dieser Runde ins Protokoll* |
| **BA 7** | **Die Dichte am Finger** *(F14–F17)* — `.pill` schrumpft im Finger-Abschnitt, und die **Auswahlfelder fallen aus der Zoomregel** | **Keine Regel fällt weg.** *Die Zusage bei `testbench.js:1893` wird **neu geschrieben** und nicht gelöscht (Stolperstein 201).* **`.icon-btn` behält seine 44 Pixel** |
...
| **2** | Sie trägt **genau die vier** Dinge aus F1 — namentlich, nicht gezählt | ein fünftes danebenstellen |
...
| **7** | **`Bild auf` und `Bild ab` blättern, die Pfeiltasten NICHT** — sie bleiben bei den Bildern | die Pfeiltasten mitnehmen |
| **8** | **Keine Wischgeste** — in derselben Ansicht wischt schon die Bildreihe | eine anhängen |
| **9** | **Die Protokollzeile bleibt bei 1024 px in ihrer Karte** — an der Karte gemessen, nicht am Fenster | die Behälterabfrage gegen eine Fensterabfrage tauschen |
...
| **15** | **`F_ROUTES` steht auf 73** — und die neue ist namentlich genannt | eine zweite dazunehmen |
...
| **2** | **Durch zehn Einträge blättern**, mit den Pfeilen und mit `Bild auf`/`Bild ab` |
...
| **Die siebzehn Fensterabfragen auf Behälterabfragen umbauen** | *ein neues Werkzeug führt man an einer Stelle ein und sieht es sich an* (F4) |
...
| `Doku/Projektstand_Kriterion_0_28_0.md` | `git mv`, **Revision 80** — dazu `F_ROUTES` 73 und die neue Kopfzeile in Abschnitt 5 |
